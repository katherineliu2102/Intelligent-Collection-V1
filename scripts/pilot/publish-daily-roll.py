...
向 intelligent-collection-schedule-v1 发布一条 job=dailyRoll 的调度消息，
等价于 scheduler 到点自动触发的日切。用于：日切未按预期触发、或灰度期手动补一次日切。

凭证：优先 GOOGLE_APPLICATION_CREDENTIALS，否则读 /opt/app/secrets/credentials.json。
支持 authorized_user 与 service_account 两种 key；不打印任何密钥。

用法：
  ./scripts/pilot/publish-daily-roll.py
"""
...
CRED = Path(os.environ.get("GOOGLE_APPLICATION_CREDENTIALS", "/opt/app/secrets/credentials.json"))
