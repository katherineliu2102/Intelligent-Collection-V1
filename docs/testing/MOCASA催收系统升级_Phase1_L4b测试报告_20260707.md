...
| L4b-5 | 快照字段来源（当时 `context_snapshot` 校验） | ✅ 通过（仅当次事实） |
...
**当次运行结论：** 核心链路（入案 → 建计划 → 扫描执行 → 渠道下发 → timeline 落库 → 还款/升档/停催/幂等）在本次环境中获得了记录的运行证据；同时发现共享订阅被其他消费者分流。此事实不能替代 T4 所需的独占订阅、真实 PubSub 还款复跑和官方闭环脚本证据。
...
### L4b-5 快照字段来源（当时运行观察）
校验 `t_contact_plan.context_snapshot`：当时观察到财务字段由 ingestion 回填后的事件 payload 映射，静态字段来自 payload；旧库是回填/对账来源，不表示引擎运行时直接读旧库建快照。
