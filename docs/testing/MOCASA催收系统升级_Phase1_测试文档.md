...
| 阶段 | 主要层级 | 当前状态 | 结论 |
|---|---|---|---|
| T0 | 环境与基线 | ✅ | T0-1～T0-7 全部闭合；T3o 准入不再受 T0 阻断 |
...
**T0-6 收尾（2026-08-25 下午，AI Call 合并后复核）**：

| 漂移 | 处置 |
|---|---|
| 设计文档定 SSOT 为 `t_ai_collection`，`CaseQueryController` 仍查旧库 `t_collection`；投了新 `caseEvent` 后后台搜不到而引擎已在催 | 检索源已切至 `t_ai_collection`，新增 `collectionStatus` / `product` 字段与筛选，计划状态改挂该案**最新一条** `t_contact_plan`（原 `ANY_VALUE` 取任意历史计划）；`CaseQueryControllerTest` 7 例锁定读源与绑定 |
| 发版手册的服务器构建流程只投递 jar，而 `deploy/Dockerfile` 已新增 `COPY deploy/certs/valubo-voice-test.crt` | 手册补齐证书与 Dockerfile 的投递步骤；照旧流程执行会在 COPY 层直接构建失败 |
| 发版手册记「闸门须同时接受 `ai`」，与 `PilotReadinessValidator` 只接受 `ai` 矛盾 | 已改为按代码口径描述，并指向「发的是半合并包」这一真实成因 |
| T5 Pilot 手册 §4.1 记「优先级为 Nacos > 环境变量」 | 按代码口径改为 **环境变量 > `application-pilot.yml` > Nacos 导入配置**，并注明 `spring.redis.*` 无法由 Nacos 轮换；`application.yml` 中同样误导的注释一并订正 |
| 真实主机与内网地址写进入库文档（发版手册、真拨记录、台账） | 统一替换为占位符，真值归 `docs/ops/生产访问凭据.local.md`（已 gitignore） |
| 7 条失效相对链接（4 条路径写错、3 条指向仓外供应商资料） | 前者修正到仓内实际位置，后者改为文字说明；全仓 `.md` 相对链接已全部可解析 |

> Nacos 优先级已按代码口径定稿，但**尚未在 Pilot 机实测**。实测探针（无副作用，一条 `curl` 判读）见 T5 Pilot 手册 §4.1，归入 T3o-2 一并执行。

**出口**：T1 依赖 T0-1；T2 依赖 T0-5；T3 依赖 T0-2～T0-5。T0-6 已闭合，T3o 准入不再受其阻断。
