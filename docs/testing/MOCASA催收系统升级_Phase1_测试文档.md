...
| ② | 系统级守卫（实时查 DB） | `PreFlightChecker` / engine / 主架构 | caseId | true/false/异常 | 通过(#5d)；不存在/已还款→false(#5a/#5b)；读失败抛出→事件 NACK 重投(#5e) | `StepExecutionOrchestrator#executeStep`（L60–63）、`PreFlightChecker#check` |
| ③ | 业务级守卫（合规 SPI，硬超时 20ms） | `ExecutionGuard`（SPI，channel） + `SpiInvoker` / 主架构调用·编排实现 | `ExecutionContext` | `GuardVerdict`(allow/block/defer) | allow→继续；静默时段 defer→重排；其他 block→SKIPPED+推进；异常/超时 fail-close→SKIPPED+推进 | `StepExecutionOrchestrator#executeStep`（L68–97）、`#markSkipped`（L202–207） |
...
| **②-S5** | ② 系统守卫·通过 | 案件存活（未还款） | `check(caseId)` | true，可触达 | `check(...)==true` | L0 | `PreFlightCheckerTest#alive_returnsTrue`（#5d） | ✅ |
| **②-S6** | ② 系统守卫·拦截 | 案件不存在/已还款 | `check(caseId)` | false | `#5a` null→false / `#5b` 已还款→false | L0 | `PreFlightCheckerTest#caseNotFound_returnsFalse`/`repaid_returnsFalse`（#5a/#5b） | ✅ |
| **②-S7** | ② 系统守卫·读失败重投 | `getCaseInfo` 抛异常 | `check(caseId)` | 异常上抛，由事件总线 NACK 重投 | `#5e` 抛出读取异常 | L0 | `PreFlightCheckerTest#readFailure_propagatesForRetry`（#5e） | ✅ |
...
| **②-S10a** | ③ 业务守卫·静默时段 | guard `defer(..., "TIME_WINDOW", nextAllowedAt)` | `executeStep` | 重排至 `nextAllowedAt`，不跳过、不推进 | step=PENDING + `trigger_time=nextAllowedAt`；plan=`STEP_SCHEDULED`；不 dispatch、不发布事件 | L0 | `StepExecutionOrchestratorTest#quietHours_deferred`（#6a） | ✅ |
...
| **关键分支** | `AdvancementDecision`：`ADVANCE_NEXT`（有下一步：任意 delay 均 `STEP_SCHEDULED` + `trigger=now+max(0,delay)`；delay=0=即刻到期由扫描器发 `PLAN_STEP_DUE`，**非**同步递归 `execute_step`，见引擎 §4.3.2 / §11-3）/ `PLAN_COMPLETED`（终态）/ `PLAN_EXHAUSTED`（发布事件→链路⑤）；无下一步→发布 `PLAN_EXHAUSTED`；STEP_WAITING 到期：步骤 COMPLETED（无 best_result→SENT_NO_RESPONSE）+ 结转 `STEP_COMPLETED` |
...
- **§11-3 delay=0「立即执行」规格↔代码↔测试（已核对修正）**：**代码**——任意 delay（含 0）均 `STEP_SCHEDULED + trigger=now+max(0,delay)`，由扫描器发 `PLAN_STEP_DUE`（非同步递归 `execute_step`）。**规格**——引擎 §4.3.2 已于 2026-07-17 校正为同一口径。**测试**——D1 L0 已闭合（`onStepCompleted_advanceNext_delayZero_schedulesWithNowTrigger`）。
...
| ⑤ | 回调处理 | `PlanLifecycleManager` / engine / 主架构 | `CHANNEL_CALLBACK` 事件 | plan `STEP_EXECUTING`（AI_CALL）或 `STEP_WAITING`（SMS DLR）：映射 `ContactResult`+step `COMPLETED`+发 `STEP_COMPLETED`；其余态返回空（静默吸收） | `plan==null || status∉{STEP_EXECUTING,STEP_WAITING}` → noop | `PlanLifecycleManager#onChannelCallback`、`#mapCallbackToResult` |
...
| **④-S5** | ⑤ 回调·非 EXECUTING/WAITING 静默吸收 | plan 已终态（如 `PLAN_CANCELLED`；不含 `STEP_WAITING`） | `CHANNEL_CALLBACK`，`result="ANSWERED"` | 静默吸收：不改 step、不发事件，返回空列表 | 返回事件 isEmpty；`updateStepStatus` 从未调用 | L0 | `PlanLifecycleManagerTest#onChannelCallback_nonExecuting_silentlyAbsorbs`（D16，2026-07-17 改用终态用例） | ✅ |
| **④-S5b** | ⑤ 回调·WAITING 收 DLR 短路 | plan `STEP_WAITING`（SMS 观察期） | `CHANNEL_CALLBACK`，`result="DELIVERED"` | 短路结转：step `COMPLETED(DELIVERED)` + 发 `STEP_COMPLETED`（不等观察期满） | `updateStepStatus(stepId, COMPLETED, DELIVERED)`；返回事件 size=1 | L0 | `PlanLifecycleManagerTest#onChannelCallback_waitingDlr_shortCircuits`（2026-07-17，拍板 A） | ✅ |
...
| **B2 DPD 日切** | 不涉及 | `DpdStageRollHandler.dailyRoll`（XXL-Job，每日 0:35 PHT；账务落库 ≥30min 后）：重算 Max DPD → `STAGE_CHANGED`（1–90 阶段变）/ `CASE_CEASED`（≥91 写 `collection_status=CEASED`） | 🟡 **骨架/占位**：`dailyRoll()` 仅 log，**无重算逻辑 + 未接 XXL-Job**（待真实化） |
...
| ② 系统守卫 | 通过 / 拦截（null、已还款、读失败重投） | ✅ / ✅ | #5d / #5a/#5b/#5e + #5 |
...
| D16 | 链路④ `onChannelCallback` plan 非 EXECUTING/WAITING → 静默吸收（④-S5；2026-07-17 改用终态用例，WAITING 改走 S5b） | 回调态拦截 | 主架构 / L0 | ✅ 已补 2026-06-22；2026-07-17 对齐拍板 A（终态吸收） |
...
| **§11-3** | ADVANCE_NEXT 且 delay=0「立即执行」（已核对修正：规格用词↔代码实现） | **代码现状**：`onStepCompleted` 的 ADVANCE_NEXT 分支对**任意 delay（含 0）** 均走 `updateStepTriggerTime(next, now+max(0,delay), PENDING)` + `updateCurrentStep` + `updatePlanStatus(STEP_SCHEDULED)`，**不存在「同步递归 execute_step」代码路径**；delay=0 时 `trigger_time=now`，由 `TriggerScanner`/`findDueSteps` 下一扫描周期即刻发 `PLAN_STEP_DUE` 驱动。**测试现状**：L0 已有 delay=0 据实断言（D1 已闭合） | ~~规格「立即 execute_step」歧义~~ → **引擎 §4.3.2 已校正**（2026-07-17）：统一写「`STEP_SCHEDULED` + `trigger=now+max(0,delay)`，delay=0 由扫描器驱动」 | 已闭合：规格 §4.3.2 + D1 L0；无需再改 |
...
| `lifecycle/PreFlightCheckerTest` | #5a/#5b/#5d/#5e | `caseNotFound_returnsFalse`、`repaid_returnsFalse`、`alive_returnsTrue`、`readFailure_propagatesForRetry` |
| `lifecycle/PlanLifecycleManagerTest`（29） | #15–#27 + D1/D16/D17/D18/D21/D22×2/D23/D25/D26/D28/D29-L0 + WAITING×DLR | …（原 16 例）+ `onCaseIngested_factoryNull_noPlan`（D21）、`onCaseIngested_ceasedCaseStatus_skip`/`onCaseIngested_ceasedSnapshot_skip`（D22）、`onCaseIngested_firstStepTriggerTime_andSnapshotFrozen`（D23）、`onCaseIngested_caseServiceReadFailure_propagates`（D25）、`onStepCompleted_advanceNext_delayZero_schedulesWithNowTrigger`（D1）、`prepareStepDue_waitingCarryOver_defaultSentNoResponse`（D26）、`onChannelCallback_nonExecuting_silentlyAbsorbs`（D16）、`onChannelCallback_waitingDlr_shortCircuits`（④-S5b，2026-07-17）、`onCallbackTimeout_nonExecuting_noop`（D17）、`onChannelCallback_mapsResultVariants`（D18）、`onRepaymentReceived_filterFails_stillCancels`（D28）、`onCaseCeased_cancelsActivePlanAndNoRebuild`（D29-L0） |
