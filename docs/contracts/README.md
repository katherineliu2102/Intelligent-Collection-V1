...
> **字段定义 SSOT** 见 [领域模型与数据定义](../MOCASA催收系统升级_Phase1_领域模型与数据定义.md)（枚举 §2、实体 §3、上下文 §4、SPI DTO §5、EventPayload §6）；SPI 签名见 [核心引擎 §6](../MOCASA催收系统升级_Phase1_核心引擎规格.md#6-spi-接口契约)。本目录**不重复字段类型表**。
...
**真相源优先级**：`collection-common` Java 代码 > 领域模型 > 核心引擎 §6 / 基础设施 §2/§5 > 本目录用法表 > 其他文档。
