...
> **事件 Payload（现有引用）**: EventType 枚举见 [领域模型 §6.6](./MOCASA催收系统升级_Phase1_领域模型与数据定义.md#66-eventtype内部事件类型)；传输信封见 [基础设施 §2](./MOCASA催收系统升级_Phase1_基础设施交互规范.md#2-事件总线redis-stream)；`CHANNEL_CALLBACK` 字段见 [渠道总规格 §3.3](./channel/MOCASA催收系统升级_Phase1_collection-channel总规格.md#33-channel_callback-事件-payload)。其余事件 payload 字段尚无 SSOT（计划新建数据接入与事件规格 §2，EXTRACT 见 [阶段C 审计 C3-2](./audit/MOCASA催收系统升级_Phase1_跨文档去重_机制层_阶段C审计_20260618.md)）

> **阅读指南**：本规格按"结构 → 参考 → 行为 → 契约 → 横切"组织。
> - **主架构 / 评审**：§1 引擎结构与边界 → §2 事件路由 → §6 SPI 接口契约，快速把握组件、入口与对外契约。
> - **引擎实现者**：§4 计划生命周期 → §5 步骤执行管线 → §3 运行时执行模型 → §7 容错与异常恢复，按落地顺序逐节实现。
> - **声明性参考**（随用随查）：§2.1 事件路由表、§6 SPI 接口与 DTO。
...
> **生命周期不变量**：`context_snapshot` 在计划创建时冻结写入 plan 行，在计划整个存活期内保持不变；仅在三个重建点（[§4.2](#42-计划创建) 新建 / [§4.4](#44-中断处理) 升档重建 / [§4.5](#45-穷尽续建) 续建）刷新。SPI 实现须知：不得依赖快照做实时判断（见 [§6.1「SPI 设计决策」](#61-接口总览) 及 [`contracts/ContextSnapshot契约对齐`](./contracts/MOCASA催收系统升级_Phase1_ContextSnapshot契约对齐_re.md)）。
...
创建时冻结 `context_snapshot`（见 [§4 生命周期不变量](#4-计划生命周期与状态机) note）。
...
> 字段级快照 SSOT 与读约定见 [`contracts/ContextSnapshot契约对齐`](./contracts/MOCASA催收系统升级_Phase1_ContextSnapshot契约对齐_re.md)。
...
| 异常场景 | 发生位置 | 恢复策略 |
|---|---|---|
| `get_case_info` / `get_context_snapshot` 读取失败 | §4.2 / §4.5 | NACK → 重消费（案件数据是创建计划的前提） |
