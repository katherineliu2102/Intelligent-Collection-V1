...
本节定义引擎的**运行时骨架**：事件如何进入与路由（§1.1）、调度线程与 Consumer 线程如何隔离（§1.2）、并发与重复执行下如何保证一致性（§1.3）。事件 Payload 契约见 [数据接入与事件规格 §2](./MOCASA催收系统升级_Phase1_数据接入与事件规格.md#2-事件总线契约)；SPI 调用时机见 [§4](#4-spi-接口完整定义)。
...
8 个事件按计划在生命周期中的角色分为四域（引擎侧视角，不涉及上游产生方式）：
...
按 §3.1 管线顺序，字段定义如下（`//` 后为说明；完整枚举见 [领域模型](./MOCASA催收系统升级_Phase1_领域模型与数据定义.md)）：

```java
// ── 输入：引擎组装，SPI 实现方只读 ──
// ⚠ plan / currentStep 为引擎内部实体的引用，SPI 实现方不得调用任何 setter；
//   违反此约束会导致引擎状态不可预期。Phase 2 考虑替换为不可变 View 对象。
ExecutionContext {
    ContactPlan plan;                    // 当前计划（状态、阶段、案件引用）
    ContactPlanStep currentStep;         // 当前待执行步骤
    ContextSnapshot contextSnapshot;     // 案件入库快照，决策唯一输入（零 DB I/O）
    List<ContactTimeline> contactHistory; // 近期触达历史，默认最近 50 条
}

// ── ③ ExecutionGuard.evaluate() 输出 ──
GuardVerdict {
    boolean allowed;                     // true=放行
    String blockedReason;                // 拦截原因（allowed=true 时为 null）
    String blockedRuleType;              // FREQUENCY_LIMIT | TIME_WINDOW | CONNECT_AND_STOP | ABANDONMENT_RATE …
}

// ── ④ StepResolver.resolve() 输出 → ⑤ ChannelGateway.dispatch() 输入 ──
StepCommand {
    ChannelTypeEnum channelType;         // SMS | PUSH | EMAIL | VIBER | WHATSAPP | AI_CALL | TTS | HUMAN_CALL
    String targetAddress;                // 手机号 / Token / 邮箱（按渠道解释）
    String templateId;                   // 模板 ID（策略选定，执行层渲染）
    String idempotencyKey;               // 透传 step.idempotency_key，渠道层供应商去重
    Map<String, Object> metadata;        // 扩展字段，已知 key（Phase 1）：
                                         //   "stage"          → String（StageEnum.name()）
                                         //   "language"       → String（ISO 639-1，如 "tl" / "en"）
                                         //   "callbackUrl"    → String（异步渠道必填）
                                         //   "timeoutMinutes" → Integer（异步渠道回调超时）
}

// ── ⑤ ChannelGateway.dispatch() 输出 → AdvancementPolicy 输入 ──
// success 由渠道层根据 contactResult 设置：FAILED 类 → false，其余 → true
// 引擎仅读 success 决定是否进入 §3.2 故障降级；AdvancementPolicy 读 contactResult 做业务决策
StepResult {
    boolean success;                     // 渠道层是否成功接受并处理了请求
    ContactResultEnum contactResult;     // DELIVERED | CONNECTED | NO_ANSWER | REJECTED | FAILED …
    String errorCode;                    // 失败时统一错误码（success=true 时为 null）
    boolean retryable;                   // 网络超时=true；号码无效=false（仅 success=false 时有意义）
    String providerMsgId;                // 供应商消息/通话 ID，回调关联与对账
}

// ── AdvancementPolicy.decide() 输出（枚举，三选一）──
enum AdvancementDecision {
    ADVANCE_NEXT,    // → 注册下一步 Job 或立即执行
    PLAN_COMPLETED,  // → 计划终态
    PLAN_EXHAUSTED   // → 发布 PLAN_EXHAUSTED 事件
}

// ── ExhaustionPolicy.handle() 输出 ──
// 各 action 的字段约束：
//   REBUILD  → templateId 必填, targetStage = null
//   ESCALATE → targetStage 必填, templateId = null
//   COMPLETE → templateId = null, targetStage = null
ExhaustionResult {
    ExhaustionActionEnum action;         // REBUILD | ESCALATE | COMPLETE
    StageEnum targetStage;               // 仅 ESCALATE：升档目标阶段
    String templateId;                   // 仅 REBUILD：新计划模板 ID
    String reason;                       // 决策理由，写 timeline / 日志
}
```
...
| 事件反序列化失败（payload 畸形） | 跳过 → DLQ + 告警（不可重试） |
