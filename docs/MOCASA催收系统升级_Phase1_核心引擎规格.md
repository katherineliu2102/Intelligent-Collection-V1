...
核心引擎与渠道编排层的模块边界：`StepExecutionOrchestrator`（非 `EventConsumerDispatcher`）经 SPI / `ChannelGateway` 串联策略子层与执行子层（接口契约见 [§6](#6-spi-接口契约)）。下图为 [§5](#5-步骤执行管线) 步骤 ③④⑤ 的跨模块视角——`StepCommand` 由策略子层 `StepResolver` 产出、引擎持有后再传入执行子层（签名见 [§6.1](#61-接口总览)，DTO 见 [§6.2](#62-共享-dto-定义)）：
...
> **读图**：仅展示跨模块步骤 ③④⑤；①②⑥⑦ 在引擎内完成，见 [§5](#5-步骤执行管线)。
...
下表是 Dispatcher 消费并路由的**事件唯一权威清单**（Phase 1 共 9 行）：处理动作与详见均以本表为准；事件按生命周期域的分组见 [§2.2](#22-生命周期派生总览) 派生总览图。
...
| `CHANNEL_CALLBACK` | 更新步骤结果 → 发布 `STEP_COMPLETED` | [§4.3.3](#433-channel_callback) |
| `CALLBACK_TIMEOUT` | 回调超时 → 标 `FAILED` → 发布 `STEP_COMPLETED` | [§4.3.4](#434-callback_timeout) |
| `STEP_COMPLETED` | 推进决策：注册下一步 / 计划完成 / 发布穷尽 | [§4.3.2](#432-step_completed) |
...
> **③ 要点**：`REPAID > CEASED > STAGE_UPGRADE > 非终态` 为语义/审计参考，非运行时覆盖规则。投诉冻结（[§5 ②](#5-步骤执行管线)）为非终态停住，与本条正交。
...
**入/出态**：入态 `PENDING` / `STEP_SCHEDULED` → 出态 `STEP_EXECUTING`；推进后 `STEP_SCHEDULED`（**`STEP_SCHEDULED` 的唯一正常入口，见 [§4.3.2](#432-step_completed)**）。
...
下方流程图展示 §4.3 四条入口如何汇聚到 `STEP_COMPLETED` → §4.3.2；线程隔离见 [§3.1](#31-线程隔离trigger-to-event)，锁外触达见 [§5](#5-步骤执行管线)。

```mermaid
flowchart TB
    subgraph cron ["Cron 调度线程（生产事件）"]
        J1["扫 trigger_time"] --> E1["PLAN_STEP_DUE"]
        J2["扫 timeout_time"] --> E4["CALLBACK_TIMEOUT"]
    end

    subgraph consumer ["Consumer 业务线程（消费 + 执行）"]
        E1 --> S1["§4.3.1 PLAN_STEP_DUE"]
        S1 -->|"A · PENDING / STEP_SCHEDULED<br/>步骤到期，触达"| EX["execute_step（§5）"]
        S1 -->|"B · STEP_WAITING<br/>观察期满，不触达"| SC

        E2["CHANNEL_CALLBACK"] --> S3["§4.3.3 CHANNEL_CALLBACK"]
        E4 --> S4["§4.3.4 CALLBACK_TIMEOUT"]

        EX --> SC["publish STEP_COMPLETED"]
        S3 --> SC
        S4 --> SC

        SC --> S2["§4.3.2 STEP_COMPLETED"]
        S2 -->|ADVANCE| NEXT["注册下一步 / 立即执行"]
        S2 -->|PLAN_COMPLETED| DONE["终态 ✓"]
        S2 -->|PLAN_EXHAUSTED| EXH["§4.5"]
        NEXT -.->|循环| E1
    end
```

> **场景 A / B**（§4.3.1 分流）：同一 `PLAN_STEP_DUE` 事件，按**当前计划状态**走不同路径——**A** 真正触达（`PENDING`/`STEP_SCHEDULED` → 锁外 `execute_step`）；**B** 观察期结转（`STEP_WAITING` → 标记步骤完成并直接 `publish(STEP_COMPLETED)`，不等渠道回调）。

#### 4.3.1 PLAN_STEP_DUE

锁内按计划态分流（场景 A/B）：**A 到期执行**（`PENDING`/`STEP_SCHEDULED` → `STEP_EXECUTING`，锁外 `execute_step`）/ **B 观察期结转**（`STEP_WAITING` → 标记完成并 `publish(STEP_COMPLETED)`，不触达）。
...
def on_step_completed(plan, completed_step):
    ...
    if decision == ADVANCE_NEXT:
        ...
        if next_step.delay_minutes > 0:
            ...
            execute_step(plan, next_step)          # 立即执行（并发安全由 §5 ⑤½ 复检保证）
...
def on_channel_callback(event):
    ...
    publish(STEP_COMPLETED)                            # 事务外发布，保证 step 状态已落盘
```

#### 4.3.4 CALLBACK_TIMEOUT

AI_CALL 在 `STEP_EXECUTING` 等待 Webhook；回调永久不到达时计划会卡死。采用两级保障，职责不同、互补而非重复：
...
进入 `STEP_EXECUTING`（异步渠道）时注册超时 Job（[§5 步骤⑦](#5-步骤执行管线)）：

> **Timeline 落库**：超时记录 timeline 由 admin 超时扫描/Webhook 对账路径落库；引擎**仅**更新 step 状态并发布 `STEP_COMPLETED`。
...
**默认 60 分钟**（`engine.step.callback_timeout_minutes`，见 [基础设施附录](./MOCASA催收系统升级_Phase1_基础设施交互规范.md#附录运行配置与环境)）：面向 AI_CALL。等待期不占 Consumer 线程——由 Cron 扫 `timeout_time` 触发本事件；防重复拾取靠计划/步骤状态，无需拉长幂等锁 TTL（15min）。
...
| MySQL 不可达（PreFlightChecker） | ② | 静默退出，不触达 | fail-close | — |
