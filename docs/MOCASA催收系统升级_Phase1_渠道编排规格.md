...
> **关联文档**: [核心引擎规格](./MOCASA催收系统升级_Phase1_核心引擎规格.md)、[基础设施交互规范](./MOCASA催收系统升级_Phase1_基础设施交互规范.md)、[架构设计](./MOCASA催收系统升级_Phase1_架构设计文档.md)、[PRD](./MOCASA催收系统升级_Phase1_产品需求文档_PRD.md)、[引擎对齐待办](./MOCASA催收系统升级_Phase1_渠道编排与引擎对齐待办.md)、[collection-channel 总规格](./MOCASA催收系统升级_Phase1_collection-channel总规格.md)、[LTH SMS](./MOCASA催收系统升级_Phase1_LTH_SMS对接说明.md)、[SendGrid Email](./MOCASA催收系统升级_Phase1_SendGrid_Email对接说明.md)、[LTH Voice](./MOCASA催收系统升级_Phase1_LTH_Voice对接说明.md)、[FCM Push](./MOCASA催收系统升级_Phase1_FCM_Push对接说明.md)、[行业调研报告](../../AI%20collection/MOCASA催收策略编排_行业调研报告_v1.md)、代码交接 [HANDOFF.md](../HANDOFF.md)
...
> 渠道**执行层**（LTH / SendGrid / FCM API、Webhook、Adapter）见 [collection-channel 总规格](./MOCASA催收系统升级_Phase1_collection-channel总规格.md) 及四份子渠道说明。本节仅界定 **编排策略在 Phase 1 的裁剪**，与本仓库 `collection-channel` 模块（见 [HANDOFF.md](../HANDOFF.md) 模块 A）的 `DefaultPlanFactory` / SPI 替换范围一致。
...
| **Email 模板** | SendGrid 控制台 Dynamic Template；`scriptSlot` → `template_id` 见 channel 总规格附录 A | — |
...
**S4 末衔接**：`S4_EMAIL_PRE_CLOSE`（D+75）须预告——**D+91 起将完全停止一切主动催收**。
...
- **Offer**：S2 起 `offer_eligible`；F10 规则引擎匹配减免档位，**写入 `context_snapshot` 供 StepResolver 做模板变量替换**（不在 StepResolver 内调外部接口，见 [引擎对齐待办 #13](./MOCASA催收系统升级_Phase1_渠道编排与引擎对齐待办.md)）
...
架构约定：计划创建时由 **数据接入层** 将案件标签序列化写入 `t_contact_plan.context_snapshot`，运行时 SPI **只读快照、不实时查库**（见架构设计 §1.3.2）。
...
| **日切不靠 PLAN_EXHAUSTED** | DPD 变阶段由 ingestion 发布 **`STAGE_CHANGED`**（见 [对齐待办 #2](./MOCASA催收系统升级_Phase1_渠道编排与引擎对齐待办.md)），非「每日步骤跑完 → REBUILD」 |
...
| Stage | DPD     | 日骨架（未还）                           | Tone          | Email 里程碑                    |
| ----- | ------- | --------------------------------- | ------------- | ---------------------------- |
| S0    | D-3~D0  | 08 Push/SMS×4；D0 Email            | STANDARD      | D0 友好提醒                      |
| S1    | D+1~3   | 08 SMS / 2 AI / 12 Push           | STANDARD      | D+1、D+3 里程碑 + D+1 条件 Email   |
| S2    | D+4~15  | 同 S1 时间表 + Offer                  | STANDARD/FIRM | D+4/7/12 里程碑 + 条件 Email      |
| S3    | D+16~30 | **同 S2 时间表** + Pay Now 加重         | STANDARD/FIRM | D+16/23/30 里程碑 + 条件 Email    |
| S4    | D+31~90 | D+31~~60 同 S2；**D+61~~90：1 AI/日** | STANDARD/FIRM | D+31/45/60/75 里程碑 + 条件 Email |
...
| 日块       | scriptSlot                | 内容                      |
| -------- | ------------------------- | ----------------------- |
| D+31     | `S4_EMAIL_ENTRY`          | 进 S4 催告函；offer_eligible |
| D+45     | `S4_EMAIL_FINAL_REMINDER` | Final reminder 级        |
| D+60     | `S4_EMAIL_MID`            | 严重逾期 + offer            |
| D+75     | `S4_EMAIL_PRE_CLOSE`      | 预告 D+91 起 **完全停催**（零主动触达） |
...
| ------------- | ------------------------------------------ | ----------------------- | ------- |
| **里程碑 Email** | Stage 内固定 DPD 日 **14:00**，到日即发             | 固定节奏，不依赖打开率             | ✅ |
...
与核心引擎/基础设施的 **需引擎侧实现项** 见 [引擎对齐待办](./MOCASA催收系统升级_Phase1_渠道编排与引擎对齐待办.md)（仅列引擎负责人待确认内容）。业务对照见 [行业调研报告 §10.2](../../AI%20collection/MOCASA催收策略编排_行业调研报告_v1.md)。
...
| §4.2 D+91 停催 | 事件 **`CASE_CEASED`**；引擎 §2.4 见 [对齐待办](./MOCASA催收系统升级_Phase1_渠道编排与引擎对齐待办.md) |
