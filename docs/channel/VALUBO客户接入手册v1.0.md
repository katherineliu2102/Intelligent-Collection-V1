# VALUBO 批次外呼客户接入手册 v1.0

> **文档状态**：与现行生产 API 对齐（内部维护；大变动后再交付客户）  
> **创建日期**：2026-06-20  
> **最近修订**：2026-09-09（催收结果升级为六层契约，并收紧本通结果与案件历史资料的边界；销售结果契约不变）
...
`dial_policy.timezone` 只用于判断本地拨打窗口，以及催收结果中解析「明天」「周五」等本地日期；不会改变 API / Callback 时间戳。`timestamp`、`dial_timeline` 等回传时间统一使用 UTC（`+00:00`）。
  ...
  "cases": [
    {
      ...
      "earliest_dial_at": "2026-06-20T02:00:00+00:00",
      "business_context": {
        "borrower": {
          "name": "Ana Reyes",
...
- `result` 是贵方提供的历史摘要标记，不是本通 Callback 的结果字段；平台只校验 `prior_contacts` 为数组，不校验该字符串的枚举。
- 若历史来自新版催收 Callback，建议在有值时直接写本通 `disposition`；未形成催收收口时用贵方自己的线路/身份标记，并在 `summary` 里写清。
- 历史联系、历史承诺和协商选项只用于本次对话参考。只有对方在本次通话中明确引用或确认相应内容时，才会进入本通 `disposition` / `promises`；平台不会仅因案件中存在历史承诺，就把它当成本通新承诺。
...
| 值 | 含义 |
| --- | --- |
| `promise_to_pay` | 本人形成单笔还款承诺 |
| `payment_arrangement` | 本人谈妥分期 |
| `refused` / `disputed` / `hardship` / `callback` / `unresolved` | 本人通话的其它新版催收收口 |
| `no_answer` / `voicemail` / `call_screening` | 贵方自定义的历史线路标记；这些不是 `disposition` |
  ...
  "data": {
    ...
    "integration_result": {
      "ai_result": {
        ...
        "summary": "Borrower confirmed identity and promised to pay PHP 500 by Friday.",
        "promises": [
          {
            "amount": 500.0,
            "currency": "PHP",
            "promised_date": "2026-06-26",
            "status": "logged"
          }
        ]
      },
...
- `status` 为**案件状态**（见 §9.2）。
- 未接通时先读 `failure_class`，再读 `final_failure_reason`；接通时二者都为 `null`。
...
不要把所有结果压成一个标签。按下面六步读；上一步不成立，后面字段就会为空：

1. **电话线接通了吗？** 看 `line_outcome.was_answered`。信箱和筛选助理也已经接通，所以也是 `true`。
2. **为什么没接通？** 仅失败通话先看顶层 `failure_class`，再看 `final_failure_reason`（案件查询中对应 `failure_reason`）。
3. **接通后对面是谁？** 看 `line_outcome.party`：`human`、`voicemail`、`call_screening`。
4. **真人有没有实际开口？** 看 `ai_result.effective_conversation`。至少有一句对方非空转写才是 `true`。
5. **是不是借款人本人？** 进入模型评估后看 `ai_result.right_party`：`"yes"`、`"no"`、`"unknown"`。
6. **催收最后怎么收口？** 只有 `right_party="yes"` 才读 `ai_result.disposition`，且只可能是 §9.6 的七个值之一。

`line_outcome.reason` 是本通会话的线路结果；`failure_reason` / `final_failure_reason` 是案件最终失败细码。失败时两者通常相同，但客户归因应以上述顶层失败字段为准。
...
| `party` | 接通后对面是谁：`human` / `voicemail` / `call_screening`；未接通为 `null` |
| `sip_code` | 可选。未接通失败时常见（如 486 / 406 / 487）；接通收口可能没有 |

### 9.5 失败归属与细码（案件 `failed` 时）

案件终态为 `failed` 时，先用 `failure_class` 看责任层，再用 `failure_reason` / Callback `final_failure_reason` 看细因。接通成功时两个字段都为 `null`。未知的新细码默认归入 `our_system`，避免误算成客户拒接。

| `failure_class` | 人话含义 | `final_failure_reason` / `failure_reason` 精确取值 |
| --- | --- | --- |
| `callee` | 对方号码或话机给出的业务结果 | `BUSY`、`NO_ANSWER`、`DECLINE`、`INVALID_NUMBER`、`TEMP_UNAVAILABLE` |
| `network` | 线路、媒体协商或运营商链路问题 | `MEDIA_NEGOTIATION_FAILED`、`SIP_SERVER_ERROR`、`FAILED`、`FORBIDDEN`、`REQUEST_TIMEOUT` |
| `our_system` | 我方没有把电话正常打出或正常收口 | `MISSING_CALLER_CLI`、`NO_CALLER`、`NO_ACTIVE_LINE`、`SESSION_START_FAILED`、`DIAL_ERROR`、`SIP_PLACE_CALL_FAILED`、`MEDIA_WS_PREMATURE_CLOSE`、`MEDIA_TIMEOUT`、`RECONCILE_TIMEOUT`、`RECONCILE_INTEGRATION_PREMATURE`、`ADMIN_MARKED_FAILED`，以及未知新码 |

手工重拨时把上表“精确取值”列中的细码放入 `failure_reasons` 即可过滤；未列出的细码也可原样传入过滤。

### 9.6 催收对话结果

`ai_result` 的五个字段为 `effective_conversation`、`right_party`、`disposition`、`summary`、`promises`。`disposition` 仅在「真人开过口且确认为本人」时有值，七个值互斥：

| `disposition` | 人话含义 |
| --- | --- |
| `promise_to_pay` | 最后形成单笔还款承诺；承诺金额或日期至少有一项 |
| `payment_arrangement` | 最后谈妥可描述的分期方案 |
| `refused` | 认账，但明确不还或不谈还款 |
| `disputed` | 否认债务、金额或合法性，或声称已还 |
| `hardship` | 认账但暂时无力偿还，且没有形成方案或承诺 |
| `callback` | 约定了具体回电时间或时间窗口 |
| `unresolved` | 本人已沟通，但以上结果都不成立 |

判定边界：

- 本次对话决定本通结果；案件金额、历史承诺和 Agent 单方面提出的方案不能独立形成 `disposition` 或 `promises`。
- 单笔 PTP 至少有金额或日期。只说 “I will pay” 但没有金额、日期或明确接受的具体方案，归 `unresolved`。
- “I will pay Friday” 是日期型 PTP；“I will pay everything” 可使用案件逾期额作为全额承诺。
- 有明确金额或日期的条件承诺（如 “If my salary arrives, I will pay Friday”）可归 PTP。
- `try`、`maybe`、`should be able` 等试探表达不算可执行承诺，归 `unresolved`。
- Agent 提出具体金额或日期后，只有对方明确接受才形成承诺；仅回复 `Okay`、表示考虑或回避，不算接受。
- 前后结论冲突时，以最后仍成立的结论为准：先拒后诺可以是 PTP；先诺后拒则是 `refused`，旧承诺不保留。
- 最终不是 PTP 或分期时，`promises` 固定为空数组。

摘要 `summary` 用英文一至三句说明过程；承诺明细放在 `promises`。销售域不使用这套催收字段，仍保留旧的 `result_label` / `summary` / `promises`。
...
| `line_outcome.party` | `voicemail` 或 `call_screening` |
| `ai_result.effective_conversation` / `right_party` / `disposition` | 均为 `null` |
| `ai_result.summary` | 固定英文文案。信箱：`Call reached voicemail; voicemail message left.`；筛选助理：`Call intercepted by a call-screening assistant; voicemail message left.` |
| `ai_result.promises` | `[]` |
| `media.recording_url` / `script_url` | 接通案通常有（见 §11.5） |

若需对信箱 / 筛选助理再联系：同批用 §6.5（`include_completed` + `completed_reasons`），或新建批次。

**真人接通率分子请用 `line_outcome.party="human"`**。`was_answered=true` 只说明线路接通，包含信箱和筛选助理，不能直接当真人接通分子。
...
| `failure_class` | 未接通时的失败归属：`callee` / `network` / `our_system`；接通时为 `null` |
...
| `ai_result` | 催收：`effective_conversation` / `right_party` / `disposition` / `summary` / `promises`；销售仍为旧 `result_label` / `summary` / `promises` |
...
```json
{
  ...
  "ai_result": {
    ...
    "summary": "The borrower discussed repayment but made no actionable commitment.",
    "promises": []
  },
  "failure_class": null,
  "final_failure_reason": null
...
`summary` 固定含 `total` / `completed` / `failed`。催收批次的其余键按各案件**当前 `session_id`** 对应的 `ai_result.disposition` 计数；旧重试 session 不重复累计。销售批次仍按当前 session 的旧 `result_label` 计数。线路维度（忙线、未接、信箱等）请以逐案 `session.completed` 或案件查询为准。
...
11. 报表区分线路接通与真人接通；线路接通看 `was_answered`，真人接通看 `party=human`（见 §9.3、§10）。
...
- [ ] 催收结果已按 `effective_conversation` / `right_party` / `disposition` / `summary` / `promises` 新结构落库，不再读取旧 `result_label`。
- [ ] 已区分 `promise_to_pay`、`payment_arrangement` 和 `unresolved`，不会把含糊或试探性付款表达直接记为 PTP。
