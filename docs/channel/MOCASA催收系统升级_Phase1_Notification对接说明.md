...
> **状态**：供 `NotificationSmsAdapter` / `NotificationPushAdapter` 实现参考；与 [引擎渠道执行契约对齐_待编排确认](./MOCASA催收系统升级_Phase1_引擎渠道执行契约对齐_待编排确认.md) 对齐，**待编排拍板后冻结**。`errorCode` 仅落 timeline，引擎不解析。
