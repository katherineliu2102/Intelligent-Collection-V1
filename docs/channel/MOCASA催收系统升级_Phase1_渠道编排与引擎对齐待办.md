...
```
每日 0:35 PHT（账务数据落库至少 30 分钟后）dpdStageRollHandler
  → 重算 Max DPD
...
| 方案           | 做法                                                  | 优点                                       | 缺点                        | Phase 1  |
| ------------ | --------------------------------------------------- | ---------------------------------------- | ------------------------- | -------- |
| **E2-A（推荐）** | 仅 `**dpdStageRollHandler`** 日批（0:35 PHT；账务落库 ≥30min 后） | 与现有 `planStepDueHandler` 风格一致；易批量处理 D+91 | 升阶最长约 24h 滞后（降阶/还款仍靠实时事件） | **默认**   |
