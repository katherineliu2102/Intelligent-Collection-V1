...
|------|---------------------------|----------------|------|
| App Push | 通知中心 → JPush | `FcmPushAdapter`（占位）→ **`NotificationPushAdapter`** | **P0** 按 [Notification §2](./channel/MOCASA催收系统升级_Phase1_Notification对接说明.md)；需 App 提供 **jpushToken** |
| 催收 SMS | 通知中心（内部路由 QH/Hiway/BORI） | `LthSmsAdapter`（占位）→ **`NotificationSmsAdapter`** | **P0** 按 [Notification §1](./channel/MOCASA催收系统升级_Phase1_Notification对接说明.md) |
...
| 5 | FcmPushAdapter + SMS fallback | ✅ 占位代码 | **待改为 NotificationPushAdapter** + 通知中心联调 TC-PUSH-01/02 |
...
| 优先级 | 工作项 | 预估 | 验收 | 阻塞 |
|--------|--------|------|------|------|
| **P0** | **NotificationSmsAdapter + NotificationClient** | 1d | TC-SMS-01；`requestSuccess=true` | 通知中心 `appCode`/`appKey` |
| **P0** | **NotificationPushAdapter**（替换 FcmPushAdapter） | 1d | TC-PUSH-01/02；入队即完成 | App **jpushToken** ingestion |
