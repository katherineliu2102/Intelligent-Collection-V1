...
- **Catch-up（错过夜间窗口）**：仅指 **策略重建** `force=true` 时 `futureSlots(now)` 丢掉**已过的今日槽**、从下一槽铺新计划。**不是**到期扫描把昨日 `PENDING` 补打出去。过日步与已过下一产品槽由引擎 `MISSED_SLOT`（[2026-09-14 修订](./MOCASA催收系统升级_Phase1_过日槽不作废补打_20260914.md)）。发版并热加载模板后，用 `force=true` 重建。例如 9/12 09:45 重建，当日不再铺 08:00/09:15，从 **11:30** 起铺新五波剩余槽。已接通案件仍走 CONNECT_AND_STOP。接口默认仍拒绝 03:35–19:00；catch-up 必须显式 `force=true`。
