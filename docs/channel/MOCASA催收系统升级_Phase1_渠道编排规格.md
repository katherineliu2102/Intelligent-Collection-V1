...
| **渠道**    | Push + SMS + Email + AI 外呼；Viber Phase 1 不做                     | 渠道选型与业务确认；IM 通道后移 Phase 2                                |
...
D+1~3   S1：08 SMS / AI 五波 09:15–18:40 / 12 Push / D+1 Email
...
**机器轨**：无需坐席干预的自动化触达（Push / SMS / Email / **AI 外呼**）。  
**LTH 域外**：TTS 单向播报、人工外呼（坐席/预测式）由 LTH 现网独立编排，与本系统无交互。
...
| **第 1 次 / 同日后续外呼**  | 当日首次 AI 外呼 / 未 CONNECT_AND_STOP 时的后续波次（09:15 起最多五波） | 真人有效沟通后当日不再后续 AI |
...
| 外呼调度          | S1+ 外呼槽 **09:15 / 11:30 / 14:30 / 16:15 / 18:40**（`trigger_time`）；S4 D+61~90 仅 **09:15**；Phase 1 **不**要求引擎实现 VoiceQueue；并发由 **LTH** 排队（滚动为 LTH/Phase 1.5 能力） |
...
flowchart TB
  ...
  MP --> AI[09:15+ AI外呼 ≤5次/日；S4 D+61~90 仅1次]
  MP -.同案不双呼.-> HP
```
...
|-------------------|--------------|----------------|
| `DISPUTE` / `COMPLAINT_FROZEN` | cancel 当日该案 SCHEDULED/EXECUTING 的 **AI_CALL** steps；释放 `voice_lock` | 写 `human_dial_override`；机器轨 Guard **BLOCK** 常规 SMS/Push/Email |
...
- `ExecutionGuard.CONNECT_AND_STOP`：仅挡 **机器常规** 同日剩余 AI 外呼；**不挡** Override=true 时 LTH 核实外呼。
...
| **接通但未还** | 不算回收；**当日不再打剩余 AI 外呼**（CONNECT_AND_STOP 跳过同日后续槽）；短信/Push/Email 仍可发 |
| **语音日上限** | S1–S3 与 S4 D+31~60：≤5 次/日；S4 D+61~90：≤1 次/日（靠 dayBlocks 少铺槽，不靠日限=1）。AI+人工合计，**Override 核实性外呼可不计入催收上限** |
...
| S1    | D+1~3   | 08 SMS / 5 AI / 12 Push           | STANDARD      | D+1、D+3 + 条件      | **D+1** ✅ |
| S2    | D+4~15  | 同 S1 时间表 + Offer                  | STANDARD/FIRM | D+4/7/12 + 条件    | **D+4** ✅ |
| S3    | D+16~30 | **同 S2 时间表** + Pay Now 加重         | STANDARD/FIRM | D+16/23/30 + 条件  | **不发** ❌ |
| S4    | D+31~90 | D+31~~60 同 S2（5 AI）；**D+61~~90：1 AI/日** | STANDARD/FIRM | D+31/45/60/75 + 条件 | **D+31、D+75** ✅ |
...
| ------ | --------- | ----------------------------------------------------- |
| 08:00  | SMS       | 首触；短链还款                                               |
| 09:15  | AI Wave-1 | 当日第 1 次外呼                                               |
| 11:30  | AI Wave-2 | 未 CONNECT_AND_STOP 则拨                                     |
| 12:00  | Push→SMS  | 午间 Push；无 token 或投递失败则改 SMS                           |
| 14:00  | Email 里程碑 | **D+1**：首次正式逾期通知；Phase 1 **不发** D+3                     |
| 14:30  | AI Wave-3 | 未 CONNECT_AND_STOP 则拨                                     |
| 16:00  | 条件 Email  | Phase 1 不发（§7.9）                                          |
| 16:15  | AI Wave-4 | 未 CONNECT_AND_STOP 则拨                                     |
| 18:40  | AI Wave-5 | 未 CONNECT_AND_STOP 则拨；须在 21:00 静默前收口                    |
...
| `per_case_daily_cap` | 5 次/案/日（S4 D+61~90 模板只铺 1 槽） |
| `retry_min_interval` | ≥4h                            |
| Wave-1 已接通           | 当日不再后续 AI 波（CONNECT_AND_STOP） |
...
| 09:15 / 11:30 / 14:30 / 16:15 / 18:40 | AI 五波；真人有效沟通后 CONNECT_AND_STOP |
| 12:00  | Push                                      |
| 14:00  | Email 里程碑：Phase 1 **仅 D+4**              |
| 16:00  | 条件 Email：Phase 1 不发（§7.9）               |
...
| 子区间           | AI 上限      | 日结构差异                         |
| ------------- | ---------- | ----------------------------- |
| **D+31~D+60** | ≤5 次/日     | 同 S1–S3 五波                     |
| **D+61~D+90** | **≤1 次/日** | **仅 09:15**；Remedial SMS 片段     |
...
| AI 外呼 | WAIT_CALLBACK | 外呼结果分支：PTP→子流程；争议→冻结+Override；需人工跟进→Override；未接→同日后续波次 |
...
- S1~~S3 与 S4 D+31~~60：外呼 **≤5 次/案/日**；S4 D+61~~90：**≤1 次/日**（模板只铺 09:15）  
- 投诉率目标 **≤0.5‰**；触线优先砍傍晚波次  
- S0 无语音；S1~S4 AI 为主，人工仅例外池
- Pilot Guard 日限：`AI_CALL=10`，`daily-total-limit=15`（见 [五波迭代](./MOCASA催收系统升级_Phase1_迭代_AI_Call五波与日限_20260911.md)）
...
| `S1_VOICE_PRIMARY` / `S1_VOICE_RETRY` | Wave-1/2  | AI_CALL          |
...
投诉率上升时建议顺序：**砍傍晚波次 → S4 维持 1 呼 → 缩短 AI dial_window → 暂停 FIRM 实验组**。
