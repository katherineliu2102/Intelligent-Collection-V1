...
**与引擎步骤完成时机（必读）**：SMS / Push / Email 在 `ChannelGateway.dispatch` 成功即 **同步** `STEP_COMPLETED`；SendGrid `delivered`/`open` **不**用于完成 plan step。AI_CALL 保持 `STEP_EXECUTING` 直至 `CHANNEL_CALLBACK`（见 [核心引擎规格 §5.1⑦](./MOCASA催收系统升级_Phase1_核心引擎规格.md#51-execute_step-执行骨架①⑦--½)）。
