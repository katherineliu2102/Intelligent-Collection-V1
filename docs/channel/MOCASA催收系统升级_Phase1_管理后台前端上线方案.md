...
> **版本**: v1.2（2026-09-08）  
> **状态**: 已按内部评审定稿，**代码与现网 nginx 尚未改**；本文是上线操作 SSOT。**钉钉告警 jar 与本文前端上线分开发**（见 §10）。  
...
| 6 | **节奏**：代码就绪 → 发 jar → 上传 `dist/`（此时 nginx 仍 403）→ **一次** reload 完整 conf。不要两次 reload 出一个「能看页面但不能登录」的中间态 |
...
### 2.1 现状（2026-09-08 已核对代码与仓库 conf）
...
| 后端认证 | `AdminAuthenticator`（BCrypt）；账号经 `COLLECTION_ADMIN_USER` / `COLLECTION_ADMIN_PASSWORD_HASH` |
| 前端登录页 | **已有密码框**，`api.login(username, password)` 已对齐后端。旧文档 §4.1 已过时 |
| 前端 API 前缀 | **未做**。`VITE_API_BASE` 默认空；`vite.config.ts` 仍按路径散代理 |
| 登录拦截 | `AdminWebConfig` 只拦 `/cases` `/compliance` `/ops` `/admin` `/config` `/dashboard`。**`/plans/**`、`/catalog/**` 未登录可调** |
...
| 先发 jar 再开 `/api`（本方案） | 一次发版窗口（回调可能短暂 502，供应商一般会重试） |
...
不要用 PowerShell 的 `scp dist/*`（glob 易踩坑）。整目录上传：

```powershell
ssh ubuntu@$env:PILOT_HOST "mkdir -p /tmp/admin-ui-dist /opt/app/admin-ui"
scp -r dist ubuntu@${env:PILOT_HOST}:/tmp/admin-ui-dist
```
...
```bash
# 若已有上一版，先备份
sudo mkdir -p /opt/app/admin-ui
if [ -f /opt/app/admin-ui/index.html ]; then
  sudo cp -a /opt/app/admin-ui "/opt/app/admin-ui.bak.$(date +%Y%m%d%H%M)"
fi
sudo rsync -a --delete /tmp/admin-ui-dist/dist/ /opt/app/admin-ui/
ls -lh /opt/app/admin-ui/index.html
```

此时 **不要** `nginx -s reload`。公网仍应全部 `/` → 403（webhook 除外）。可用手机热点抽查 `/` 仍 403。

静态文件更新以后若只换 hash 资源，一般也不必 reload；只有改 nginx conf 才 reload。
...
| 场景 | 动作 | 是否动容器 |
|---|---|---|
| 前端页面坏了 | 把 `/opt/app/admin-ui.bak.*` rsync 回 `/opt/app/admin-ui/`。只换文件通常不必 reload | 否 |
...
| 8   | 发新前端后白屏                        | 查 `index.html` 是否 `no-store`；用户强刷。静态文件目录用 `rsync --delete`，避免旧 hash 残留造成错觉                                     |
...
## 10. 已知问题与未闭合项（2026-09-08）

本节记录评审后仍未改代码/nginx 之前就不能执行本文 §5 的点，以及文档自身不一致。**未闭合前不要按本文打开公网 `/` 与 `/api`。**

### 10.1 与钉钉告警发版拆开（已拍板）

前端约定 **永不进 jar**。「一起发」只能是同窗口做三件事：换 jar、传 `dist/`、reload nginx。钉钉 A1/A2/A3 只需要换 jar、重启催收容器，**不改 nginx、不传静态文件**。

打开管理面要动 `location /`（方案里的危险动作），失败面从「回调短暂 502」扩到「白名单 / SPA / webhook 一起出问题」。2026-09-08 拍板：**先只发钉钉 jar**；本文前端上线另开窗口，且必须先做完 §4。

同一次容器重启可以带上已合入的后台后端（看板 API、投影护栏等），那是 jar 粒度，不是「把 UI 打进镜像」。`collection-admin/ui` 与本文 nginx 片段都不进催收镜像。

### 10.2 代码前置仍未做（§4 P0）——现在发 dist 会失败或漏 PII

| 项 | 现状 | 若跳过 |
|---|---|---|
| `VITE_API_BASE=/api` | 未做。`vite.config.ts` 仍按路径散代理 | 生产请求打 `/auth/login`，被 SPA `try_files` 吃成 HTML，登录必挂 |
| 拦截器 `/plans/**`、`/catalog/**` | `AdminWebConfig` 仍未加 | 一开 `/api`，白名单网段内未登录可读时间线、计划、话术 |
| 现网 nginx | 仍仅 `/webhook/`，其余 403 | 不 reload 则域名打不开后台；reload 未完成 §4 则不安全 |

§4 写「本轮不改代码」，§5 又把改完的产物当上线步骤。状态与操作顺序矛盾：**先改仓库并本地用 `vite preview` 验证，再进入 §5。**

「这一次必须发 jar」在本文里指 **补拦截器**，不是钉钉扫描器。拦截器没做就不能开 `/api`。

### 10.3 操作步骤里会踩坑的地方

- **PowerShell 与 bash 混用**（如 `$env:PILOT_HOST` 写进 bash）。按错壳则主机变量为空或语法失败。每一步标清在哪边跑。
- **§5.2 上传路径**：`scp -r dist` 再 `rsync .../admin-ui-dist/dist/`，目录容易套一层，`index.html` 不在 nginx `root` 下会白屏。上传后必须 `ls` 到 `/opt/app/admin-ui/index.html`。
- **`proxy_cookie_flags`** 需要 nginx ≥ 1.19.3。只写在 §3.6 / §9，没有放进 §5.0 硬闸门。上线当天先 `nginx -v`，过旧则 `nginx -t` 失败、整份 conf 上不去。
- **HTTP `location /` 从现网 403 改为 301 HTTPS**。`/webhook/` 段应保持可进，但 80 根路径行为会变。reload 后立刻用 **POST** 对照 webhook（不要 `curl -I`）。变成 403/502/无响应则按 §8 回滚 nginx，不必先动容器。
- **reload 会热加载整份 conf**，即使用户觉得「没改 webhook 段」。对照失败优先回滚 nginx。

### 10.4 白名单与仓库 conf

- §1 表格目前写了办公室 / VPN 两条；附录 B 另有两条补充出口。§9 又写「已写入四条」。**ACL 以附录 B 四条为准**，上线当天按附录 B 复查，不要只抄 §1 两行。
- 白名单不是登录。办公室 NAT / VPN 出口变了会全员 403；同一出口上的访客 Wi‑Fi 能碰到登录页（靠口令 + 拦截器）。
- 仓库 `deploy/nginx/` **故意还是「仅 webhook」**，防有人提前同步打开管理面。机上 conf 与仓库会分叉；上线成功后必须回写，否则下次按仓库覆盖会把 SPA 打回 403。

### 10.5 只发 jar、不改 nginx 时（钉钉窗口）

公网行为应与发版前相同：`/` 与 `/api`、`/actuator` 仍 403；只有 `/webhook/` 进容器。健康检查打机上 `127.0.0.1:8080`，**不要**拿公网 `https://.../actuator/health` 当就绪标准（发版手册若写公网 200，在现网 403 策略下不成立）。
