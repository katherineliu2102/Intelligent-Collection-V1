...
| 原则 | 说明 |
|------|------|
| **任务驱动导航、角色控制动作** | VIEWER / OPERATOR / SYSTEM_ADMIN 共用「今日执行 / 复盘」信息架构；后端角色决定能调用哪些接口和执行哪些动作 |
...
角色能力参考 [PRD §3.1](./MOCASA催收系统升级_Phase1_产品需求文档_PRD.md#31-用户角色)，但按当前小团队收敛成三角色，避免第一版做过细权限矩阵。

**首次上线（P0）**：每位使用者独立账号，暂时统一 `SYSTEM_ADMIN`；账号通过 Pilot 环境文件配置，禁止共用 `admin`。独立账号先解决个人审计、单人停用和口令轮换，**不等于 RBAC 已实现**。当前代码只把 `role` 放进 session，后端尚未据此拒绝接口。

**上线稳定后（P1）**：实现三角色 RBAC。后端接口授权是安全边界，前端隐藏菜单只做体验同步。

| 角色 | 核心职责 | 主要模块 |
|------|----------|----------|
| `VIEWER` | 全局只读观察 | 看板、案件/计划、目录、异常与审计日志只读 |
| `OPERATOR` | 日常管理运营 | VIEWER + 话术/计划模板、普通异常处理、冻结/解冻/升级、普通评估参数 |
| `SYSTEM_ADMIN` | 系统级高危控制 | 全部权限；独占 DLQ 重放、配置版本回滚、故障注入、全局/分渠道停催与恢复 |

### 3.1 权限矩阵

| 模块 / 操作 | VIEWER | OPERATOR | SYSTEM_ADMIN |
|---|:---:|:---:|:---:|
| 数据分析、案件、计划、目录 | 读 | 读 | 读写 |
| 话术 / 计划模板、普通评估参数 | 读 | 读写 | 读写 |
| 普通异常 ACK / Resolve | 读 | 处理 | 处理 |
| 冻结 / 解冻 / 升级 | 读 | 读写 | 读写 |
| DLQ 重放、配置版本回滚、故障注入 | — | — | 执行 |
| 全局 / 分渠道停催与恢复 | — | — | 执行 |
| 系统管理与权限配置 | — | — | 读写 |

高危操作统一要求：**二次确认 + 必填原因 + 操作人审计**。Phase 1 不做双人审批流。后端逐接口和 HTTP 方法校验角色，并补匿名 401、已登录越权 403、允许角色成功三类自动测试。

紧急停催第一版先用可执行、已演练的 runbook；后台全局/分渠道按钮属于 P1。无论按钮还是 runbook，都必须记录操作者、原因、生效状态和恢复动作。

> **v1.7 角色 vs 服务商**：内部三角色与「服务不同贷款企业」是两条轴，不要绑在一起。
> - 不做三套角色看板。看板按任务切「今日执行 / 复盘」，三角色共用指标口径与页面信息架构，只控制接口和可执行动作。
...
| 子模块 | 功能 | Phase |
|--------|------|-------|
| 独立账号 | 首批由 `pilot.env` 配置多账号，统一 `SYSTEM_ADMIN`；禁止共享账号 | 首次上线 P0 |
| 三角色 RBAC | `VIEWER` / `OPERATOR` / `SYSTEM_ADMIN`，后端逐接口授权 | 上线后 P1 |
| 账号管理 UI | 创建/禁用账号、重置密码和分配角色；使用者少时先不做 | P2 |
| 操作日志 | 记录配置变更、全局切流、冻结操作（谁、何时、做了什么） | P0 |
| 高危操作保护 | DLQ 重放、配置回滚、故障注入、紧急停催仅 SYSTEM_ADMIN；二次确认 + 必填原因 | P1 |
...
| 鉴权 | Session + 拦截器；三角色 RBAC | 首次上线独立账号统一 SYSTEM_ADMIN；P1 后端逐接口授权，见 §3 |
| 前端部署 | nginx 静态托管 + `/api` 同源代理 | `releases/<id>` + `current` 软链原子发布；不把前端放进 jar |
| Pilot 可用性 | 单机单容器，人工恢复目标 30–60 分钟 | 当前不建设双机高可用；管理后台、Webhook 与引擎共进程，变更必须对照 Webhook |
| 监控边界 | 复用催收系统既有告警 | 本轮不建管理后台专属钉钉告警；技术监控后续接统一监控平台 |
...
| 录音/转写 | **服务端代理 + 访问审计；权限待媒体代理落地时单独确认** | 敏感媒体必须做后端授权，不得只靠前端隐藏；无复听即无质检 |
| 触达时段 | **08:00–21:00 + 静音窗 21:00–08:00（与 PRD §7.2 默认一致）** | v1.2 曾误引为 6:00–22:00；v1.4 核对 PRD 原文后修正，引擎与 PRD 一致 |
| 角色模型 | **首批独立账号统一 SYSTEM_ADMIN；稳定后 VIEWER / OPERATOR / SYSTEM_ADMIN** | v1.7；后端授权需开发，不能只配 role 或隐藏菜单 |
| 看板导航（v1.7） | **两视图：今日执行 / 复盘；不做三套角色页面** | 三角色共用口径与信息架构，只控制接口与动作。服务商 = `tenant_id` 隔离，与角色无关 |
