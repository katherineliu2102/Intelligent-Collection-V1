...
> **版本**: v1.6  
> **日期**: 2026-09-01  
> **状态**: ✅ 现行基线（SPA 已落地）+ AI Call 观测迭代已并入（v1.3）+ 小团队角色模式与差距地图（v1.4）+ 测试与自动跑校准（v1.5）+ 数据分析看板催收管理视角重构（v1.6）  
...
> - 测试主文档《MOCASA催收系统升级_Phase1_测试文档（链路 × 层级）》（`AI collection/archive/`）、Pilot 自动跑综述（8/29–8/31，`ic-fmt/docs/testing/`）与自动跑记录（9/1，`docs/testing/`）——测试层级与 Pilot 事实来源  
> **输入来源**: 管理后台设计讨论（2026-06-30）、业内催收 SaaS 调研、同事评审优化（2026-06-30）；v1.2 对齐新 Pub/Sub 投影与 AI Call Facade；v1.5 结合测试与自动跑记录校准；v1.6 数据分析指标重设计讨论（2026-09-01，四项决策）
...
**v1.3 并入 AI Call 观测迭代（2026-08-31）**：看板新增 AI Call 分区（§5.1.4）、单案 360 外呼增强与录音/转写代理（§5.3.2）、钉钉 CRITICAL 告警（§5.5.4，关闭 Q3）、数据底座 `t_ai_call_session`（§6.2）。该批原以独立《AI 通话运营台开发文档》承载，已合并至本文并删除——**本文为该批唯一 SSOT**；实现落地后同步回写操作手册 §3.1 / §3.4 / 告警节。
...
**v1.5 测试与自动跑校准（2026-09-01）**：以测试主文档（链路 × 层级，L0–L4b，CI 常绿 99 例）与 Pilot 自动跑记录（8/29–8/31 综述、9/1 记录）校准本文：AI Call 分区（§5.1.4）背景刷新至 9/1 波次并新增波次延迟列；接通案 513849 实证（`summary`/`result_label` 回传、`CONNECT_AND_STOP` 链上生效）更新 §5.3.2 / §5.2.4 / `needs_review` 判定样本；§5.5.4 补同进程扫描盲区（9/1 Hikari 池耗尽暴露）；§13 差距地图按 9/1 事实更新（日切自然跑通过、S0 部分进件、新增「调度失败不重投」）。

**v1.6 数据分析看板催收管理视角重构（2026-09-01）**：以催收管理者视角重设计 §5.1 指标——按业务重要性/管理决策顺序重排（回收置顶），模块化为 回收→迁徙→结果链→AI Call→风险信号→渠道 ROI，并新增口径规范（§5.1.7）；四项决策：看板信息架构不按「四问」分类法、AI Call 首屏业务结果优先、归因窗口统一 48h、高敏标签仅展示+提醒（§11）。
...
```
管理后台
├── 📊 数据分析              ← v1.6 按业务重要性排序（回收置顶）
│   ├── 回收效果看板          ← 置顶；当日回收 + 分 Stage 回收率（§5.1.1）
│   ├── 案件迁徙与老化        ← 迁徙率 / 滞留 / 待核销（§5.1.2）
│   ├── 触达与结果链          ← 场景 B 起点；四层漏斗（§5.1.3）
│   ├── AI Call 分区          ← 第一屏业务结果 / 第二屏渠道卫生层（§5.1.4）
│   ├── 风险信号              ← 高敏标签 / 断联 / 悬挂（§5.1.5）
│   └── 渠道 ROI 概览         ← 触达成本 vs 回收（P1，§5.1.6）
├── ⚙️ 策略配置
│   ├── 触达计划模板          ← Stage × 日块 × 槽位
│   ├── 话术与模板            ← scriptSlot × 渠道
...
**定位**：观测层——回答「发生了什么、哪里有问题」，是策略迭代起点与异常发现辅助入口。**v1.6 重构视角**：模块按**业务重要性 / 管理决策顺序**排序（回收置顶），每个模块服务一个真实业务场景；不再以「渠道运营视角」为主线，也不按「问题清单」组织章节。

> 六模块服务三类真实场景：**主管经营**（回收 + 迁徙 + 风险信号）、**策略迭代**（触达与结果链 + AI Call 结果分布）、**渠道运维**（AI Call 卫生层 + 渠道健康）。
...
| 层 | 指标 | 数据源 | 延迟 | 支撑 |
|----|------|--------|------|------|
| **热层（实时）** | 当日回收金额、触达量、送达率、真人接通率、result_label 分布、高敏标签计数、异常计数 | MySQL / ODS 直读（运行表聚合）+ inbox 还款事件 | 分钟级 | 15 分钟闭环、异常发现 |
| **冷层（趋势）** | 分 Stage 回收率、Stage 迁徙率、PTP 兑现率、Aging、渠道 ROI | BigQuery / 数仓 | T+1 / 小时级 | 趋势分析、策略复盘 |

> 策略配置员改话术后，先在热层看「触达/结果分布」是否即时变化（闭环验证），回收类结果在冷层次日复盘。

#### 5.1.1 回收效果看板（P0，置顶）

**服务场景**：主管经营视图——打开后台第一眼看到的「钱回来了没有」。

| 指标 | 口径 | 数据源 | 层 |
|------|------|--------|----|
| 当日回收金额 | PHT 自然日，金额口径，近 7 日均值参考线 | inbox `repaymentEvent` → 投影结清 | 热（时效实测后校准） |
| 分 Stage 回收率 | S1 ~36% / S2 ~17% / S3 ~5% / S4 ~4%（PRD §2.3 基线画目标线）；金额 + 笔数双口径 | 还款事件 + 案件表 | 冷 |
| 治愈率 | 当期还款回到正常的案件数 / 金额 | 还款事件 + 案件表 | 冷 |
| OS 在催余额趋势 | 逾期在催余额随日变化 | 案件表聚合 | 冷 |

> 基线参考线做法与 AI Call 分区 BUSY ~42% 基线一致（目视锚点，不告警）。绩效报表导出不在本系统实现（PRD §9）；看板仅供内部策略与监控使用。

#### 5.1.2 案件迁徙与老化（P1，新增）

**服务场景**：主管经营视图——「案件在往哪走」，组合健康度。

| 指标 | 口径 | 数据源 | 层 |
|------|------|--------|----|
| Stage 迁徙率 | S1→S2 / S2→S3 / S3→S4，期初 cohort 口径 | 案件表快照 | 冷 |
| 阶段内滞留 | 各 Stage 滞留天数分布 + 滞留 N 天以上案件数/金额（N 待数据校准） | 案件表 | 冷 |
| S4 待核销 | D+91 前最后阶段案件数 / 金额 | 案件表 | 冷 |

> 迁徙是方向性指标：滞留堆积 = 当前 Stage 策略失效信号；治愈（回正常）在回收看板体现。

#### 5.1.3 触达与结果链（P0，重组自「触达效果看板」）

**服务场景**：策略配置员迭代——「动作有效吗」，四层结果链（口径见 §5.1.7）。

| 层 | 指标 | 数据来源 | 层 |
|----|------|----------|----|
| L1 触达 | 发送量、送达率、真人接通率（AI Call）、已读率（Email） | `t_contact_timeline` + 渠道回调 | 热 |
| L2 沟通 | AI Call result_label 分布（业务结果 / 合规风险 / 未分类三桶） | `t_ai_call_session`（待建） | 热 |
| L3 承诺 | PTP 率、承诺金额分布（测试样本少、暂无 PTP 案例，占位展示） | `t_ai_call_session`（待建） | 热 |
| L4 行为 | 触达后 48h 还款转化（统一归因窗口） | timeline + 还款事件 | 冷 |

- 维度：渠道 × Stage × 模板/scriptSlot（**PRD F11 对齐**：SMS、AI Call 为 KPI 口径渠道；Push / Email 仅看板呈现，PRD §6.2）；时间日/周/月趋势（冷）。
- 交互：默认近 7 日、可自定义时间范围 ✅；cell 下钻 scriptSlot / 案件 ⏳ Phase 1.5；每指标标注新鲜度 ✅。
- 用户级去重触达率保留为 **PRD 对账列**（AI Call 3.4% 基线为去重用户口径）。

#### 5.1.4 AI Call 分区（重组）

**背景**（保留）：真实分流后（8/24–8/28 盘点 `t_channel_callback_audit`，149 去重会话）SIP 406 占 47%、486/BUSY 占 42%、真实接通约 0.67%，主叫号单一（`6310001`）；**9/1 复测未见改善**（09:15 批 134 通：BUSY 64 ≈48%、FAILED 40 ≈30%、NO_ANSWER 29 ≈22%、真实接通 1 ≈0.7%）——供应商修复截至 9/1 无进展，修复前基线以两期分布为准。现有触达效果按短信 `DELIVERED/FAILED` 口径，AI 的 `ANSWERED/BUSY/NO_ANSWER` 掉进 Other——故**同页独立分区，不动短信口径**，避免混用（不要用 deliveryRate 看 AI）。

**v1.6 重构为双屏**：第一屏**业务结果**（催收价值），第二屏**渠道卫生层**（供应商质量诊断）。
...
| AI 接续成功 | `was_ai_connected` 且 `line_reason=NORMAL` | 真人多轮（= 结果链 L1 触达成功口径） |
| 接通但无效 | `was_answered` 且 reason ∈ {VOICEMAIL, CALL_SCREENING} | 单独一档，防止计入有效对话 |

**第一屏 · 业务结果（v1.6，稳态默认）**：

1. result_label 分布：分母 = AI 接续成功会话；三桶（业务结果 / 合规风险 / 未分类）；未知标签不丢弃、归未分类并计入数据质量信号。
2. PTP 率 + 承诺金额分布（L3；测试样本少、暂无 PTP 案例，占位展示）。
3. 四层漏斗：终点延伸至 L2/L3，不再停在「AI 接续成功」。
4. 高敏标签（dispute 等）计数，跳风险信号看板（§5.1.5）。

**第二屏 · 渠道卫生层（供应商质量诊断）**：

1. SIP 码 + `line_reason` 分布。
2. 波次表：拨出 / BUSY / FAILED / NO_ANSWER / ANSWERED、FAILED 率与 BUSY 率、主叫号、`stage`/`dpd` 快照可筛；**含「计划槽位时间 vs 实际起拨时间」延迟列**（9/1 14:30 批因连接池耗尽晚约 7 分钟起拨，波次级延迟必须可见，见 §13.1）。
3. 主叫号 × SIP 交叉（当前仅 `6310001` 一行；扩号后直接看哪个号 486 多）。
4. 四层电信漏斗 + 每层流失 Top 原因；BUSY 趋势画 ~42% 基线参考线（目视锚点，不告警）。
...
- `needs_review`：`was_answered=1` 且（尚无转写，或转写中无借款人发言）。**判定样本已齐（9/1）**：513849（488 秒争议对话，`summary` 非空、`result_label=dispute`，应清除）与 513749（77 秒静默接通，`summary` 空，应保持）——以两案 `script_url` 转写定说话人角色判定规则（任意非空借款人 utterance 即清除）；无角色标注则退化为人工标记，`summary` 非空可作辅助清除信号。
...
**供应商并行推动（不改本仓库，带证据去谈）**：SIP 406 媒体协商、486 + 单主叫号轮换（含 local presence 拨号评估）、回调补 `answered_at`/`ended_at`/`duration` 与发言轮次字段、result_label / promises 稳定回传。修复验收出口（判据可配）：建议 406 <5%、BUSY <25%、真实接通 ≥15%——无量化出口则「修好」不可证伪，告警抑制永不解除。
...
| 指标 | 口径 | 数据源 | 层 |
|------|------|--------|----|
| 高敏标签 | dispute 等计数/占比 + 最近待处理清单（初始集合 {dispute}，配置化增补） | `t_ai_call_session`（待建） | 热 |
| 断联信号 | INVALID_NUMBER 计数；连续未接通口径待数据校准 | `t_ai_call_session`（待建） | 热 |
| 悬挂会话 | AI_CALL EXECUTING 且 `dispatched_at` 早于 15 分钟（A3 看板面） | 步骤表 | 热 |
| 合规拦截量 | Guard 拦截量（PRD 目标 100% 拦截） | 步骤表 | 热 |

> **高敏标签动作（v1.6 决策 D4）**：仅展示 + 异常队列提醒（§5.5.1 新增 `HIGH_SENSITIVITY_LABEL` 类型），**不自动暂停**；人工确认后在案件 360° 一键冻结/解冻。原因：争议案在人工处理前仍会按策略外呼，靠提醒置顶 + 快速人工冻结兜底。

#### 5.1.6 渠道 ROI 概览（P1，口径定死）

| 指标 | 说明 |
|------|------|
| 单渠道触达成本 | 供应商单价 × 触达量估算 ⏳ 成本数据需运营维护 |
| 单渠道回收贡献 | 该渠道触达后 **48h** 内还款金额 / 案件数（归因窗口 §5.1.7） |
| 成本回收比 | 辅助策略配置员判断渠道投入 |

数据来源：触达量来自 timeline；成本单价 ⏳ 待确认维护方式（配置表或手工导入）。

#### 5.1.7 口径规范（全局纪律，v1.6 新增）

| 规范 | 定义 |
|------|------|
| 统计单元（分母） | 率指标分母统一**案件（借据）**；attempt 级仅渠道诊断；用户级去重保留为 PRD 对账列 |
| 双口径 | 回收类金额口径为主、笔数并列；率类笔数口径 |
| 归因窗口 | 触达后 **48 小时**内还款计为转化，全渠道同窗（单值） |
| PTP 兑现率 | promised_date 当日（PHT）内还款（初版，待数据校准宽限） |
| 高敏标签集合 | 初始 {dispute}，配置化增补，不冻结枚举（result_label 为开放标签集） |
| Stage 与 DPD | DPD 为 Stage 同义别名，不双轴（借据级 overdue_days MAX） |
...
保存即生效；违规拦截记录可在案件 360° 或看板中查看（v1.3：看板 AI Call 分区露出窗口内 Guard 跳过步数，见 §5.1.4），不单独建设合规管理模块 ✅。投诉冻结、`CONNECT_AND_STOP`（当日接通停呼）均已在引擎侧生效——后台只做可见性，不重做合规引擎（`CONNECT_AND_STOP` 已有链上实证：9/1 案 513849 上午接通后，14:30 槽即 SKIPPED）。
...
| AI Call 会话明细 | 三层布尔、SIP、`line_reason`、`result`、`result_label` / `summary`（9/1 确认 Facade 真实接通会回传，dispute 类为审读重点）、`wave_key`、`caller_cli`、stage/dpd 快照、`needs_review`、synthetic、时长占位 | `t_ai_call_session`（v1.3，按 `case_id` 读） |
...
| A3 | 步骤 EXECUTING 且 `dispatched_at` 已过 15 分钟、明细表无对应 `session_id` | 悬挂（补 `callbackTimeout` 因 `timeout_time` 空扫不到的洞）；**同步入异常队列**供人工收敛，不只是通知。9/1 连接池耗尽后人工盯 EXECUTING 归零即此场景——A3 把这类盯梢自动化 |
...
- **同进程盲区（9/1 实证）**：Hikari 池耗尽时 planStepDue / callbackTimeout 扫描失败且不重投、health 503——A1–A6 与被监控对象**同进程、共连接池，可能同归于尽**。波次级兜底走进程外：actuator health 外部监控（GCP Cloud Monitoring / Uptime）+ 分区波次延迟列（§5.1.4）+ `format=md` 日报位；引擎侧调度重投属 §13.1 差距，不在本批。
...
- 写入：`FacadeWebhookService.handle` 双写（验签、身份与 `ContactResult` 映射逻辑不变；审计 `disposition` 改写原生词，不再硬写 NULL）；历史由 `scripts/pilot/backfill-ai-call-session.py` 按 `session_id` 幂等回填（只读审计原文；连 Pilot 写入需批准，本地/CI 用夹具）。**两套口径并存**：`t_ai_call_session` 存供应商**原生词**（AI 分区与 360 以此为准，不受引擎映射影响）；step/timeline 存映射后 `ContactResult`——引擎 `mapCallbackToResult` 对非法/空值兜底 ANSWERED，可能掩盖真实结果，该契约由测试主文档链路④ L2 差集（D4）盯，不在本文重复。
...
| 交付项 | 说明 |
|--------|------|
| 触达与结果链看板（冷热分离） | 四层漏斗（触达→result_label→PTP→48h 还款）渠道 × Stage × scriptSlot；热层实时、冷层趋势（§5.1.0/§5.1.3） |
...
| 渠道运维 | 开关、熔断、全局切流 |
| 回收效果看板（**P0 置顶**） | 当日回收 + 分 Stage 回收率 + 治愈率 + OS 趋势（§5.1.1） |
| 案件迁徙与老化（P1） | 迁徙率、阶段内滞留、S4 待核销（§5.1.2） |
| 风险信号（P1） | 高敏标签清单、断联、悬挂聚合、合规拦截（§5.1.5） |
| SQL 规则治理 | F12 |
| **AI Call 观测（v1.3 增量，本批；v1.6 加业务结果首屏）** | 看板 AI Call 分区（业务结果首屏 + 渠道卫生层）+ `t_ai_call_session` 底座与回填 + result_label 聚合 + 钉钉 CRITICAL（A1–A4+A6）+ 案件 360 录音/转写代理 + 波次延迟列（§5.1.4 / §5.3.2 / §5.5.4 / §6.2）；验收口径 = 供应商修复验证闭环（修复前基线 = 8/24–8/28 与 9/1 两期分布）；新增组件按测试主文档 L0 / L3 / L4b 登记差集，不另立测试体系 |
...
| PTP | **schema 已冻结；看板 L3 指标占位展示，落地待真实 PTP 案例** | v1.6 纳入结果链 L3（PTP 率/兑现率）；测试样本少、暂无 PTP 案例，先占位不返工 |
| AI Call 告警 | **钉钉 A1–A4+A6；n≥20 小样本不告；3 日抑制 + 恢复通知** | v1.3，Q3 关闭；抑制键 = alert_id + 槽位 HHMM |
| 波次延迟 | **波次表加「计划槽位 vs 实际起拨」延迟列，不新增 A7 告警** | v1.5；9/1 池耗尽致 14:30 批晚 7 分钟；同进程告警不可靠（§5.5.4 盲区），先做可见性，进程外 health 监控兜底 |
| 本批验收 | **供应商修复验证闭环**（修复前后 FAILED/BUSY 分布与告警可对照） | 不等 PTP/回款；建议出口判据 406 <5%、BUSY <25%、真实接通 ≥15%（可调）；修复前基线 = 8/24–8/28 与 9/1 两期（9/1 复测无改善） |
| 看板信息架构 | **按业务重要性/管理决策顺序重排，回收置顶；不以「四问」问题分类法组织** | v1.6；六模块 = 回收→迁徙→结果链→AI Call→风险→ROI，各服务一真实业务场景 |
| AI Call 首屏 | **业务结果优先**（result_label 分布 + PTP + 漏斗），SIP/电信质量降第二屏 | v1.6；接通是卫生指标、对话结果才是业务指标（稳态视角） |
| 归因窗口 | **统一 48 小时**，全渠道同窗（单值口径） | v1.6；§5.1.6「X 日」定死为 48h；窗口越长被多轮触达污染越重 |
| 高敏标签联动 | **仅展示 + 异常队列提醒，不自动暂停** | v1.6；人工确认后处置（案件 360° 冻结）；自动暂停可被话术滥用、全冻结误判代价高 |
...
| Q2 | 渠道 ROI 成本单价由谁维护、如何录入？ | §5.1.6 渠道 ROI | ❓ 待运营确认 |
...
> **先把圈画清**：Pilot 上跑通的是 Phase 1 **自动触达主链路**（进件 → 投影 → 日切 → 五槽四渠道 → Facade 回写），这还不是完整催收作业系统。自动跑（8/29–9/1）即测试主文档 **L4b（真实源 + 真实渠道）** 形态的连续实证：引擎语义已由 L0/L1/L2 覆盖（CI 常绿 99 例），暴露的缺口集中在数据源、落库与运行可靠性。本节把缺口分三层，并回答「**哪些缺口能由管理后台承接**」——后台能直接解决的是第二层全部 + 第一层的可见性；第一层的根因与第三层的大多数在引擎、上游数仓或 Phase 2。

### 13.1 第一层：主链路本身还没稳（后台只做可见性，修复在上游/引擎）

| 缺口 | 后台承接 | 形态 |
|---|---|---|
| 进件覆盖：200 圈投影 159、仍缺 41（含 D91 4 案）；S0 由整桶为零转为**部分进件**（16 进投影 / 11 在催，9/1）；根因（数仓发布条件 `overdueAmount>0`）未改 | ✅ 对账视图 | 看板新增「圈选 → 入案」对账区：订阅圈清单 vs 实际入案数，缺圈标红（数据源：ingestion 计数 + 圈选配置，只读） |
| 日切续档 9/1 03:35 **首次自然跑通过**（scanned=195、stageChanged=8、未手补）；单次通过 ≠ 稳定，需连续多日观察；穷尽升档与「走完等次日」可能双计划 | ✅ 检测 + 下钻 | 单案 360 已展示 plan history；**「同案双活跃计划」检测入异常队列**（PLAN_STUCK 变体，建议随 v1.3 批实现） |
...
| 接通质量：9/1 案 513849 为 488 秒真实争议对话（`summary` 非空、`result_label=dispute`、当日 `CONNECT_AND_STOP` 停呼生效）——**有效对话通路已打通但样本仅 1**；513749（77 秒静默、`summary` 空）仍是反例 | ✅ 已并入 v1.3 | AI Call 分区（§5.1.4）+ 360 转写代理 + `needs_review`（判定样本已齐，见 §5.1.4） |
| inbox 补发 Job 缺失（坏消息靠 Pub/Sub 重投） | ⚠️ 只做积压视图 | inbox 积压/死信计数入看板；补发 Job 属 ingestion 侧 |
| **调度扫描失败不重投（9/1 新增）**：14:35–14:37 Hikari 池打满，planStepDue / callbackTimeout 扫描失败且不重投、health 503，14:30 批晚约 7 分钟起拨（98 通） | ⚠️ 波次延迟可见 + 进程外监控 | 波次延迟列（§5.1.4）+ actuator health 外部监控（§5.5.4 同进程盲区）；重投/自愈与连接池容量属引擎，见 §13.3 |
...
| 缺口 | 后台承接 | 状态 |
|---|---|---|
| AI 运营台（按波次看 FAILED/BUSY、后台听录音读转写） | ✅ | **已并入本文 v1.3**（§5.1.4 / §5.3.2 / §5.5.4 / §6.2），未开工 |
| 转写与 summary 落库 | ✅ | 360 转写代理已设计；**Facade 回传已实证**（9/1 案 513849 非空 `summary` / `result_label=dispute`），落库列随 v1.3 批建 |
...
2. **第一层根因**（进件覆盖、日切稳定性、圈选隔离、投影死锁、调度失败不重投）在上游与引擎，后台只保证「看得见、告得出」，修复责任不在本文范围。
...
> **修订历史**  
> - v1.6 · 2026-09-01 · 数据分析看板催收管理视角重构（四项决策）：§5.1 重排为回收置顶（5.1.1）→迁徙与老化（5.1.2 新增）→触达与结果链（5.1.3 重组）→AI Call 分区（5.1.4，第一屏业务结果/第二屏渠道卫生层）→风险信号（5.1.5 新增）→渠道 ROI（5.1.6 口径定死）+ 口径规范（5.1.7 新增）；归因窗口统一 48h；高敏标签仅展示+提醒；PTP 入 L3 占位；§4.1 导航、§10.2 路线、§11 决策表、§12 Q2 联动  
> - v1.5 · 2026-09-01 · 测试与自动跑校准：§5.1.4 背景刷新至 9/1 波次（BUSY ~48% 无改善）并新增波次延迟列；接通案 513849 实证更新 `summary`/`result_label` 展示（§5.3.2）、`CONNECT_AND_STOP`（§5.2.4）与 `needs_review` 判定样本（§5.1.4）；§5.5.4 补同进程扫描盲区、A3 实证注记；§6.2 明确原生词/映射词两套口径（映射兜底风险由测试主文档 D4 盯）；§13 差距地图按 9/1 事实更新（日切自然跑通过、S0 部分进件、新增调度失败不重投）并与测试主文档 L4b 挂钩；验收基线锚定两期分布  
...
> - v1.3 · 2026-08-31 · 并入 AI Call 观测迭代（原《AI 通话运营台开发文档》合并至本文后删除）：看板 AI Call 分区与指标口径（§5.1.4）、案件 360 外呼增强与录音/转写代理（修订「不内嵌录音播放」）、钉钉 CRITICAL A1–A4+A6（Q3 关闭）、`t_ai_call_session` 数据底座与回填约定（§6.2）、触达窗以引擎口径为准（Q9）、`promises_json` schema 冻结  
