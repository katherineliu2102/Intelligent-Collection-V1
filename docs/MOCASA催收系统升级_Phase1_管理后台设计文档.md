...
**定位**：观测层——回答「发生了什么、哪里有问题」。看板按「谁在看、要回答什么问题」分**经营 / 催收 / 策略**三层**正交**视图：同一数据模块只归属一个视图，内容互不重叠。是策略迭代与异常发现的入口。

> **v2 重写说明（2026-09-04）**：本节按「三层正交视图」整体重写，替代旧版「触达效果看板 + AI Call 分区 + 回收/ROI 平铺」结构。旧版问题：三层视图共用同一批模块导致内容重叠、`queryByStage` 把 SMS/PUSH/EMAIL 合并按 Stage 统计（违反 P1）、Aging 错位放进催收执行视图。重写后模块归属见 §5.1.1，开发必须严格遵守，不得偏离。
...
| P4 | **三层视图内容正交** | 经营 / 催收 / 策略三视图各自回答独立问题，同一数据模块只归属一个视图，不重复出现。 |
...
| 视图 | 核心问题 | 内容性质 | 默认时窗 |
|------|----------|----------|----------|
| **经营** | 池子多大？收回来多少？资产健康吗？ | 结果（存量 + 回收 + 健康度） | 时点 + 近 7 日 |
| **催收** | 今天触达了多少？接通了多少？有没有异常要处理？ | 执行（今日触达 + AI 结果 + 异常） | 今日（实时） |
| **策略** | 哪个渠道在哪个 Stage 转化差？计划跑得正常吗？ | 质量（维度下钻 + 趋势） | 近 7 日 |

**正交性**：每个数据模块只归属一个视图，判断依据 =「这个指标主要帮助谁做哪类决策」。

| 模块 | 归属视图 | 回答的问题 |
|------|----------|-----------|
| 资产组合快照 | 经营 | 池子多大、今日进出多少 |
| 回收成效 | 经营 | 收回来多少 |
| 队列与迁徙（Aging） | 经营 | 资产健康度 |
| 今日触达执行（分渠道） | 催收 | 今日各渠道打了多少、送达/接通如何 |
| AI Call 业务结果 | 催收 | 今日 AI 接通多少、有无有效对话 |
| 风险信号与异常 | 催收 | 有无要人工处理的 |
| 渠道 × Stage 矩阵 | 策略 | 哪个渠道在哪个 Stage 差 |
| 计划执行分布 | 策略 | 计划跑得正常吗 |
| 渠道质量趋势 | 策略 | 哪个维度在恶化、要不要调 |

#### 5.1.2 经营视图（结果大盘）

回答「催收业务整体经营结果」。面向管理层，不涉及具体渠道执行细节。
...
> **Aging 归属说明**（2026-09-04 确认）：DPD 桶分布反映资产组合健康度与迁徙趋势，是经营视角（管理层看大盘），**不放催收执行视图**——催收执行关心「今天打了多少」，不是资产健康度。

> **回收口径说明**：绩效报表导出不在本系统实现（[PRD §9.1](./MOCASA催收系统升级_Phase1_产品需求文档_PRD.md#91-绩效报表)）；看板仅供内部策略与监控使用。

#### 5.1.3 催收视图（今日执行与异常）

回答「今天催收执行得怎么样、有没有要处理的」。默认时窗**今日（PHT 实时）**，面向一线运营。
...
回答「策略配置的执行质量如何、哪里要调」。默认时窗**近 7 日**（便于看出趋势），面向策略配置员。
...
| 波次 | `batch_id` | — |
| Stage / DPD | `stage_snapshot` / `dpd_snapshot`（会话发生时快照） | 修复前历史数据为空显示 `—`；**禁止用投影现值回填展示** |
| 接通时间 | `answered_at`（供应商 `dial_timeline`） | 未回传显示「未回传」，**禁止显示 0 或 epoch** |
| 时长 | `ended_at - answered_at` 派生（秒） | 任一为空显示 `—`，**禁止显示 0** |
| 结果标签 / 摘要 | `result_label` / `summary` | 空显示 `（无标签）` / `—` |
| 主叫 | `caller_cli` | — |

**G. SKIPPED（未入波）分类**——L0 与 L1 的差值必须可解释，按原因分组：Guard 拦截（合规/频次）/ 已结清 / 出窗（投影 dpd 超窗）/ **CONNECT_AND_STOP**（同日已接通案件的当日后续 AI 步骤 SKIPPED；**跨日可再拨**，9/3 实证 `529588` 9/1、9/2 接通后 9/3 下午再通）/ 频控上限（如 S4 dpd≥61 仅 1 通/日）。
...
| 看板信息架构 | **三层正交视图：经营=结果大盘 / 催收=今日执行与异常 / 策略=质量调优** | v1.5，§5.1 重写；同一数据模块只归属一个视图，内容不重叠 |
| 触达聚合同径 | **不聚合异质渠道（P1 第一约束）** | v1.5；SMS/PUSH/EMAIL/AI_CALL 触达语义不同，禁止合并按 Stage 统计，须分渠道或渠道×Stage 矩阵 |
| Aging 归属 | **经营视图（资产健康度），不放催收执行视图** | v1.5，2026-09-04 确认；DPD 桶是经营视角，催收执行关心「今日触达」非资产健康度 |
...
| # | 问题 | 影响 | 状态 |
|---|------|------|------|
| Q1 | 看板冷热分离细节（热层物化视图刷新周期） | §5.1.0 架构已定为 MySQL 热 + BQ 冷；刷新策略待实现 | ⏳ 待深入讨论：默认热层 1–5 分钟刷新 |
| Q2 | 渠道 ROI 成本单价由谁维护、如何录入？ | §5.1.3 渠道 ROI | ❓ 待确认：待运营提供维护方式 |
| Q4 | Grafana 嵌入后台还是跳转独立页面？ | §5.4.2 | ⏳ 待深入讨论：默认跳转，嵌入成本高 |
