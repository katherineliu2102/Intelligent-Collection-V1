...
> **版本**: v1.6  
> **日期**: 2026-09-07  
> **状态**: ✅ 已确定（设计基线）；v1.6 收口看板两视图 / 告警 A1–A3 先做 / 后台替代按日自动跑记录。未开工项见正文 ⏳ / ❓。实现进度见 HANDOFF 与测试文档。  
...
| 模块 | 归属视图 | 回答的问题 |
|------|----------|-----------|
| 五槽收口时间线（NEW，替代自动跑 §2） | 今日执行 | 08:00 / 09:15 / 12:00 / 14:00 / 14:30 各槽打了多少 |
...
回答「今天五槽有没有收口、这波该不该叫人」。默认时窗**今日（PHT）**，打开页面现查热层。本视图是停止撰写按日自动跑 Markdown 的前提（完整对照见 §5.1.7）。

**模块 0：五槽收口时间线**（NEW，区间型，热层）——自动跑记录 §2 的产品化

固定五行，缺波显示 `—`（Email 无里程碑库存时发送=0 属正常，不标红）：

| PHT 槽 | 渠道 | 必列数字 |
|--------|------|----------|
| 08:00 | SMS / PUSH | DELIVERED；SMS 附加 Stage 拆分（仅该渠道内，不跨渠道） |
| 09:15 | AI_CALL | 实拨 / ANSWERED / BUSY / FAILED / NO_ANSWER / SNR / SKIPPED；`batch_id` |
| 12:00 | PUSH | DELIVERED |
| 14:00 | EMAIL | DELIVERED + SKIPPED；slot 拆分（如 `S2_EMAIL_ENTRY`） |
...
#### 5.5.4 告警联动（v1.6：先落地 A1–A3；规格仍含 A4/A6）
...
**本批必做（A1–A3）**——阈值入 `t_evaluation_setting`，`config_type=alert`；未实现前不得把本文成功标准「5 分钟入队」标成已交付：

| ID | 条件 | 意图 |
|---|---|---|
| A1 | 波次 FAILED 率 >15% 且真实 completed ≥20 | SIP 406 类媒体故障；9/7 全日 FAILED ~32% 即应在 09:15 收口后响起 |
...
| AI Call 告警 | **本批钉钉 A1–A3（含 A3 入异常队列）；A4/A6 后做；n≥20；3 日抑制** | v1.6 纠正 v1.3「Q3 已关闭」——代码尚未实现，不得当已交付 |
