...
| 完成信号 | Phase 1 **不**要数仓完成信号。零 NEW 案与 Publisher 故障无法从消息流区分：当日无任何案件刷新归属日（投影 `owner_date = 当日` 计数为零）则推迟对账，**不得**把空收当成「今天零案」。**08:00 PHT 仍无水位 → 钉钉 R1 CRITICAL**（到期扫描门控；不补打昨日槽）。补救是重发**当天** NEW / 消化积压与 DLQ |
...
3. **08:00 PHT** 仍无对账水位：钉钉 R1，今日触达门控，过日步 `MISSED_SLOT`。**06:00 PHT** 前未完成对账或日切，按 Runbook 排查 Publisher、案件订阅、投影消费与 Redis 游标（可早于 08:00 人工介入）。已过的今日槽不对账晚到而补打。
...
| 2026-09-14 | 08:00 仍无对账水位钉钉 R1；过日槽不补打；补救仅重发当天 NEW / 消化积压 | §5 / 运维协同 |
