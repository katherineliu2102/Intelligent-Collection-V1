...
- 范围：状态机（§2）+ 七步管线（§3.1）+ 系统守卫（§3.1②）+ SPI 硬超时（§4.1）。
- 现状：**55 单测全绿**（含 `SpiInvokerTest` 5 例：超时→`SpiTimeoutException`/异常透传/MDC 传递/直连不超时/正常返回），场景明细见引擎矩阵（七步管线 #1-13/#28-32、状态机 #14-27、PreFlight #5a-5e）。
- 约定：全 mock 不连库；SPI 调用经 `SpiInvoker` 硬超时（`Future.get`）+ 异常兜底，纯逻辑测试用 `SpiInvoker.direct()` 直连保证确定性。
