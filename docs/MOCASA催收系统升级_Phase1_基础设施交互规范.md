...
**实现选型** ✅：Spring Boot 2.1 不提供 Redis Stream 标准抽象（`StreamMessageListenerContainer` 在 2.2+ 才引入），消费循环基于 **Lettuce 原生 API** 手工实现：
...
| ACK 机制 | 业务处理成功后显式 `XACK`；处理失败不 ACK → pending list → 重投递；不可重试 → DLQ |

**PEL 拾取机制**：进程在 `XACK` 前崩溃时，消息滞留 PEL，新实例仅读 `>` 会永久漏触达。消费者启动时及 PEL Scanner 定期执行（频率与规格见 [§1](#1-消费线程模型)）：

```
1. XPENDING <stream> <group> - + COUNT
2. 对 idle > pel_idle_minutes 的消息执行 XAUTOCLAIM（或 XCLAIM），转移至当前消费者
3. 重新投入消费管线处理（幂等保护保证安全重试）
```

> PEL idle 阈值默认 10 分钟（`engine.consumer.pel_idle_minutes`），须大于单条消息最长处理时间，防止误抢活跃消息。

**看门狗机制**：手写消费循环存在连接假死风险（Lettuce 连接断开但线程无异常退出）。线程规格见 [§1](#1-消费线程模型)，核心逻辑：

| 组件 | 行为 |
|---|---|
| 消费线程 | 每次 `XREADGROUP` 轮询后更新心跳时间戳（Redis `SET` 或内存变量） |
| 守护线程 | 心跳超时（`watchdog.timeout_seconds`，默认 60s）时：① 先中断旧消费线程（`interrupt()` + 等待终止）；② 重建 Lettuce 连接和消费上下文；③ 触发告警（[运维与协作](./MOCASA催收系统升级_Phase1_运维与协作.md)） |

> 重建前必须先终止旧线程，防止旧连接（网络卡顿非真死）与新连接并存导致双重消费。
...
| `callbackTimeoutHandler` | `0 * * * * ?`（每分钟） | `t_contact_plan_step WHERE timeout_time <= NOW() AND status = 'EXECUTING'` 且关联计划为非终态 | `CALLBACK_TIMEOUT`（[核心引擎规格 §2.3.4](./MOCASA催收系统升级_Phase1_核心引擎规格.md#234-异步回调超时兜底)） |
| `ptpExpiredHandler` | `0 0 * * * ?`（每小时） | 到期且未处理的 PTP 记录 | `PTP_EXPIRED` |
...
| PTP到期 | PTP_EXPIRED | isRepaid, getPtpRecord, findActivePlan | updatePtpStatus, savePlan |
...
| getPtpRecord(ptpId) | PTP 记录 | 只读 | t_collection_ptp_info |
| updatePtpStatus(ptpId, status) | PTP 状态更新 | 事务 | t_collection_ptp_info |
...
> Phase 1 不引入 Apollo/Nacos，DB 轮询足够（参数变更频率极低）。
...
|---|---|---|---|---|
| `engine.step.idempotency_ttl_minutes` | `15` | Y-注意 | 步骤幂等锁 TTL（分钟）。改小有新老交替期风险 | [核心引擎规格 §3.1](./MOCASA催收系统升级_Phase1_核心引擎规格.md#31-executestep-七步管线) 步骤① |
| `engine.step.max_retry_count` | `3` | Y | 步骤渠道发送最大重试次数 | [核心引擎规格 §3.1](./MOCASA催收系统升级_Phase1_核心引擎规格.md#31-executestep-七步管线) 步骤⑥ |
| `engine.step.retry_base_interval_seconds` | `30` | Y | 首次重试退避基准（秒） | [核心引擎规格 §3.1](./MOCASA催收系统升级_Phase1_核心引擎规格.md#31-executestep-七步管线) 步骤⑥ |
| `engine.step.retry_max_interval_seconds` | `300` | Y | 退避上限（秒） | [核心引擎规格 §3.1](./MOCASA催收系统升级_Phase1_核心引擎规格.md#31-executestep-七步管线) 步骤⑥ |
| `engine.step.retry_backoff_factor` | `2` | Y | 退避倍数（第 n 次等待 = min(base × factor^n, max)） | [核心引擎规格 §3.1](./MOCASA催收系统升级_Phase1_核心引擎规格.md#31-executestep-七步管线) 步骤⑥ |
...
| `engine.spi.exhaustion_policy.timeout_ms` | `50` | Y | ExhaustionPolicy 硬超时（毫秒） | [核心引擎规格 §4.1](./MOCASA催收系统升级_Phase1_核心引擎规格.md#41-接口总览) |
...
| `engine.step.callback_timeout_minutes` | `60` | Y-注意 | 异步渠道回调超时（分钟）。改小有新老交替期风险 | [核心引擎规格 §2.3.4](./MOCASA催收系统升级_Phase1_核心引擎规格.md#234-异步回调超时兜底) |
| `engine.context.history_max_records` | `50` | Y | ExecutionContext.contactHistory 最大记录数 | [核心引擎规格 §4.2](./MOCASA催收系统升级_Phase1_核心引擎规格.md#42-共享-dto-定义) |

> 渠道编排侧的配置参数见 [渠道编排规格](./MOCASA催收系统升级_Phase1_渠道编排规格.md)。
