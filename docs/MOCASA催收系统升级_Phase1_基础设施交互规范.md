...
> **范围**: 仅覆盖菲律宾市场  
> **模块**: —  
> **SSOT 分工**：本文 **§2 / §3 / §5** 为 EventBus、Redis 运行时键、Repository 方法语义的文档 SSOT。表结构与字段见 [领域模型](./MOCASA催收系统升级_Phase1_领域模型与数据定义.md)；SPI/DTO 见 [核心引擎 §6](./MOCASA催收系统升级_Phase1_核心引擎规格.md#6-spi-接口契约)。索引见 [架构 §1.1](./MOCASA催收系统升级_Phase1_架构设计文档.md#11-架构总览)。  
