...
> **配套执行文档**（2026-09-03 增）：[开发计划](./testing/MOCASA催收系统升级_Phase1_按日Owner路由开发计划.md)（四轨道分工 / G1、G2 灰度闸门 / 已拍板决策）、[老系统改造需求](./老系统_按日Owner路由改造需求.md)（提交老系统团队）、[数仓改造清单](./数仓_按日Owner路由改造清单.md)（提交数仓团队）
...
5. 到达对账时刻后发布内部 `CASE_OWNER_RECONCILED`；若当日仍无任何 NEW `caseEvent` 落库（投影无任何案件刷新归属日），推迟对账并告警，不得把空收当成「今天零案」。
...
- owner 以每日批次为准、批后不得翻案：当日迟到补发（原决定即 NEW、老系统未触达）允许当日回迁；已判 LEGACY 的案件如需改判 NEW，次日批次生效。
- Phase 1 不对账数仓完成信号。对账时刻固定为 **03:35 PHT**。零 NEW 案与「Publisher 故障」无法从消息流区分，须以空收检测（当日无任何案件刷新归属日）与告警兜底。

> 🔄 **2026-09-02 修订**：新增「批后不得翻案」条——堵住当日迟到补发 / 已迁出案件的中途回迁造成同日双触达（违反 §1.2「同一 `caseId` 同一自然日 owner 不变」）。
...
- 灰度期间白名单从「选案依据」降级为「应急隔离开关」：生效口径为「白名单 ∩ owner=NEW」双门控，用于路由异常时一键止血；正式分流稳定后删除白名单，仅保留数仓 owner 单一路由。白名单不得作为长期 owner 路由来源。

> 🔄 **2026-09-02 修订**：白名单原表述为「保留原有接入/扫描白名单作为隔离护栏」；现明确其灰度期角色为应急隔离开关（白名单 ∩ owner=NEW 双门控）且正式分流稳定后删除——选案权威移交数仓 owner 后白名单不再承担选案职能。
...
| 日期 | 变更 |
| --- | --- |
| 2026-09-02 | 初版（评审稿：缺席对账 + 归属日 + 水位门控） |
| 2026-09-02 | 评审结论落地：§4.2 新增「批后不得翻案」规则（堵同日双触达）；§7.2 白名单降级为应急隔离开关（白名单 ∩ owner=NEW，正式分流后删除）；明确 Phase 1 不做批次完成信号、老系统侧改造不在本计划范围 |
| 2026-09-03 | SSOT 先行落地：领域模型（`CASE_OWNER_RECONCILED` / `ROUTED_TO_LEGACY` / payload 行）、核心引擎（路由表 / §4.4 迁出与重入路径 / §4.2 幂等标注）、数据接入（`owner_date` 刷新 / 日切第一阶段）、数仓契约（`owner` 字段 / 仅发 NEW / 完成信号口径）——详见各文档「变更记录」 |
| 2026-09-03 | 配套执行文档三份：[开发计划](./testing/MOCASA催收系统升级_Phase1_按日Owner路由开发计划.md)（四轨道 / G1、G2 闸门）、[老系统改造需求](./老系统_按日Owner路由改造需求.md)、[数仓改造清单](./数仓_按日Owner路由改造清单.md)；同日拍板：跨系统频控 Phase 1 = 已知限制 + 监控 |
| 2026-09-03 | §3.1 / §4.2 空收兜底表述与实现对齐：空收检测以投影 `owner_date = 当日` 计数为准（走索引、时区口径与投影写入一致），不再对 inbox payload 做字符串日期解析（见数据接入规格 §4.1）；本文与 `docs/testing/` 旧工作副本内容同步 |
