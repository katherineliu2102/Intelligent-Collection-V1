 ...
 *
 * <p>实时查 DB 确认案件存在且未还款。与业务级 ExecutionGuard 的区别： 本类是系统级不变量（案件还活着吗），失败 → 静默退出（不记录、不推进）。
 *
...
public class PreFlightChecker {
    ...
    public boolean check(Long caseId) {
        return blockingReason(caseId) == null;
    }

    /** 业务性阻断必须有可持久化的终态原因；基础设施读取异常仍向上抛以触发 NACK。 */
    public CancelReason blockingReason(Long caseId) {
        CaseInfo info = caseService.getCaseInfo(caseId);
        if (info == null) {
            log.info("[PreFlight] caseId={} not found, skip", caseId);
            return CancelReason.CASE_NOT_FOUND;
        }
        if (info.isRepaid()) {
            log.info("[PreFlight] caseId={} already repaid, skip", caseId);
            return CancelReason.REPAID;
        }
        return null;
    }
}
