 ...
 *
 * <p>把"线程级强制超时 + 异常解包 + MDC 跨线程传递"集中到一处，
 * 替代原先散落在 Orchestrator / LifecycleManager 的 5 段裸调，
 * 让 5 个 SPI 的失败语义可单测、可冻结。
 *
 * <p><b>职责边界</b>：本类只负责"在阈值内拿到结果，否则抛异常"。
 * fail-close（Guard→SKIPPED、Resolver→FAILED）与 NACK（PlanFactory/Advancement/Exhaustion）
 * 仍由调用方的 try-catch 决定——超时被转为 {@link SpiTimeoutException}，
 * 运行时异常原样上抛，二者对调用方表现一致。
 *
 * <p><b>限制（不夸大）</b>：{@code Future.cancel(true)} 是尽力而为，
 * 卡死在不可中断 I/O（如 socket read）的实现不一定能被真正终止。
 * 因此 I/O 型 SPI（如 ExecutionGuard 的 Redis Lua）仍须自带 client 级超时作第一道防线，
 * 本调用器是保护 Consumer 线程池不被拖垮的统一兜底。
 *
 * <p>{@code engine.spi.timeout-enabled=false} 时退化为直连（不提交线程池、不强制超时），
 * 用于本地调试与纯逻辑单测（见 {@link #direct()}）。
 */
@Component
public class SpiInvoker {
    ...
    public SpiInvoker(EngineProperties props) {
        ...
        this.timeoutsMs = t;
        this.pool = cfg.isTimeoutEnabled()
                ? buildPool(props.getConsumer().getThreadPoolSize())
                : null;
