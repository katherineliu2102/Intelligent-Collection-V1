...
>>>>>>> origin/ca_branch
    ...
    public List<ContactRecord> timeline(
            @PathVariable Long userId, @RequestParam(defaultValue = "50") int limit) {
        return timelineRepository.getContactHistory(userId, limit);
    }

    /**
     * 渠道编排只读观测聚合：案件活跃计划 + 各计划步骤序列 + 用户近期 timeline。 供 {@code static/orchestration.html} 单页展示，无写操作。
     */
    @GetMapping("/overview/by-case/{caseId}")
    public Map<String, Object> overviewByCase(
            @PathVariable Long caseId, @RequestParam(defaultValue = "50") int timelineLimit) {
        ...
        result.put(
                "stage", info == null || info.getStage() == null ? null : info.getStage().name());
